import { LOGIN, PROMO_COUNTER, NOTIFICATION, USER_NOTIFICATION_STATUS, CHANGE_PROFILE_PIC, CHANGELANGUAGE, CHANGE_NAME, ADD_SUBSCRIBTION, UPDATE_SUBSCRIBTION } from '../../common/endpoints'
        ...
        Api('post', LOGIN, formdata).then((response) => {
            if (response && response.status && response.statusCode == 200) {
                if (response.userinfo && response.userinfo.language)
                    dispatch({ type: 'LOGIN_FETCHING_SUCCESS', user: response.userinfo, fcmToken: body.fcmToken, isPremium: response.userinfo.isPremium, isSocialLogin: response.userinfo.isSocialLogin })
                else
                    dispatch({ type: 'LOGIN_FETCHING_FAILED', errorMessage: "something went wrong" })
            }
            else if (response && response.statusCode && response.statusCode == 100) {
...
export function fetchChangeName(updateusername) {
    ...
    return function (dispatch) {
        ...
        Api('post', CHANGE_NAME, formdata).then(async (response) => {
            if (response.status === true) {
                dispatch({ type: 'CHANGE_NAME_SUCCESS', response: response, fullname: updateusername })
            }
            else
...
export function sendFCMToken(fcmToken, osType, uniqueId, forced) {
    ...
    return function (dispatch) {
        Api('post', NOTIFICATION, formdata).then(async (response) => {
            if (response.status === true) {
                dispatch({ type: 'SEND_FCM_TOKEN_SUCCESS', response: response, fcmToken: fcmToken, uniqueId: uniqueId })
            }
            else
...
export function resetFirstLogin(data) {
    return { type: 'CHANGE_FIRST_LOGIN', locale: data.locale, isFirstLogin: data.isFirstLogin }
}
...
export function updateSubscrition(email) {
    ...
    return async function (dispatch) {
            ...
            const valllid = await validateReceiptIos(receiptBody, false)
                ...
                .then(receipt => {
                    ...
                    if (expiration > Date.now()) {
                        console.log('inapp valid', renewalHistory[renewalHistory.length - 1])
                        ...
                        console.log('inapp un-valid', renewalHistory[renewalHistory.length - 1]);
        ...
        formdata.append('transactionDate', purchases && purchases[0].transactionDate)
