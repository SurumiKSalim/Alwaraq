...
import { BG_GRAY_COLOR, PRIMARY_COLOR, TEXT_GRAY_COLOR, WHITE_COLOR, SECONDARY_COLOR } from './../../../assets/color'
import { MaterialIndicator, BarIndicator } from 'react-native-indicators'
import { fetchCountryList, fetchShippingInfos } from '../../AddressManager/actions'
...
const App = ({ locale, shipping, dispatch, navigation }) => {
    ...
    const handler = useCallback(_.debounce((quantity, item) => onChange(quantity, item, 'add'), 1500), []);
        ...
        Api('post', BUY_NOW, formdata).then(async (response) => {
            ...
            if (response) {
                if (action == 'confirmCart') {
                    ...
                    console.log('wwww')
                    navigation.navigate('PaymentWebView', { orderId: response.orderId, totalPrice: amount, refCode: promoCode })
                }
            }
        })
    }
        ...
        return (
            <TouchableOpacity style={styles.cardGrid}>
                ...
                <View style={styles.actionContainer}>
                    <TouchableOpacity onPress={() => navigation.navigate('AddressManager', { fromPurchase: true })} style={styles.remove}>
                        <Text style={styles.removeText}>{I18n.t("Change_Address")}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => fetchInfo('delete', item.shippingId)} style={styles.remove}>
        ...
        return (
            <TouchableOpacity style={styles.cardGrid}>
                ...
                <View style={styles.priceContainer}>
                    ...
                    <PriceSubView textLabel={I18n.t("Discount")}>
                        - USD 0
                    </PriceSubView>
                    <PriceSubView textLabel="Total">
                        USD {priceData?.totalPriceUSD}{'\n'}
                        <Text style={styles.addSubText}>(AED {priceData?.totalPriceAed})</Text>
                    </PriceSubView>
                </View>
            </TouchableOpacity>
        )
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                {data && data.length > 0 &&
                    <View style={styles.dataContainer}>
                        ...
                        {shipping &&
                            shipping.length != 0 ?
                            addressView(shipping[index]
                                ? shipping[index] :
                                shipping[0]) :
                            <TouchableOpacity
                                onPress={() => navigation.navigate('AddressManager', { addressEditor: 0 })}
                                style={styles.addContainer}>
                                <Text
                                    style={styles.addContainerText}>
            ...
            <View style={styles.bottomTab}>
                <View style={styles.bottomTxtContainer}>
                    {enablePurchase &&
                        <Text style={styles.priceSubTxt}>Total : USD {priceData?.totalPriceUSD}{'\n'} (AED {priceData?.totalPriceAed} )</Text>}
                </View>
                {enablePurchase ?
                    <TouchableOpacity
                    ...
                    <View
                        style={[styles.bottomButtom, { opacity: .5 }]}>
                        <Text style={styles.bottomTxt}>Place order</Text>
                    </View>
                }
            </View>}
