...
import { StyleSheet, View, TouchableOpacity, Text, Image, TextInput, SafeAreaView, Platform } from 'react-native'
import { connect } from 'react-redux'
import { GoogleSignin } from '@react-native-community/google-signin';
import { LoginManager, GraphRequest, GraphRequestManager, } from "react-native-fbsdk"
import { PRIMARY_COLOR, SECONDARY_COLOR, TITLE_COLOR } from '../../../assets/color'
import { FONT_SEMIBOLD, FONT_LIGHT, FONT_REGULAR, FONT_MEDIUM } from '../../../assets/fonts'
...
class App extends Component {
    ...
    onSignUp() {
        ...
        if ((this.state.password == this.state.confirmPassword) && (this.state.email !== null) && this.state.emailValidate) {
            this.props.dispatch(signup({ name: this.state.name, email: this.state.email, password: this.state.password, fbgoogle: 0 }, this.props.navigation))
            ...
            <SafeAreaView style={styles.container}>
                <KeyboardAwareScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
                    ...
                    <View style={styles.header}>
                        <Text style={[styles.subtitle, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{I18n.t("Enter_your_informations_below_or_login_with_social_account")}</Text>
                    </View>
                    ...
                    <View style={styles.email}>
                        <TextInput
                            ...
                            placeholderTextColor={TITLE_COLOR}
                            onChangeText={(text) => this.setState({ email: text }), (text) => this.validate(text, 'email')}
                            style={styles.textField}
                    ...
                    {!this.state.emailValidate && <Text style={styles.emailValidate}>{this.state.disposalMail ? "This mail domain can't be used" : "Enter a valid mail"}</Text>}
                    ...
                    {this.state.error && <Text style={styles.validationText}>{this.state.error}</Text>}
                    <TouchableOpacity onPress={this.onSignUp} style={styles.button}>
                        {this.props.isLogging ? <MaterialIndicator color='white' size={20} style={styles.signUpIcon} /> :
                    ...
                    <TouchableOpacity onPress={this.login_fb} style={styles.facebook}>
                        ...
                        <Text style={styles.signinwith}>{I18n.t("Sign_Up_With")}<Text style={styles.social}>{I18n.t("Facebook")}</Text></Text>
                    </TouchableOpacity>

                    <TouchableOpacity onPress={this.signIn} style={styles.google}>
                        <Fontisto name='google' size={18} color='white' />
                        <Text style={styles.signinwith}>{I18n.t("Sign_Up_With")}<Text style={styles.social}>{I18n.t("Google")}</Text></Text>
                    </TouchableOpacity>
