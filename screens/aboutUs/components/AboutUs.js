...
import { StyleSheet, Clipboard, View, TouchableOpacity, Linking, Dimensions, FlatList, Text, Image, TextInput, ScrollView, SafeAreaView, Platform } from 'react-native'
...
class App extends Component {
    ...
    share(isAlwaraq) {
        if ((this.state.evAppInfo && !isAlwaraq) || (isAlwaraq && this.state.appInfo)) {
            ...
            var googleStoreLink = evAppInfo.googleStoreLink ? evAppInfo.googleStoreLink : ''
    ...
    render() {
        ...
        // TranslatorConfiguration.setConfig(ProviderTypes.Google, 'AIzaSyBSw_HJq3rvBjexWlpjKEms-WInGbhpUhc', 'fr');
        return (
                ...
                {!this.state.isLoading ?
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.coverImgContainer}>
                            <LinearGradient style={[styles.card, { height: height / 5 + 60 }]} colors={['rgba(0,0,0,.2)', 'rgba(0,0,0,.2)', 'rgba(0,0,0,.2)']}>
                                <Image style={styles.card} resizeMode='stretch' source={Images.alwaraqBanner}></Image>
                            </LinearGradient>
                        </View>
                        <View style={styles.contentContainer}>
                            <View style={styles.evContainer}>
                                <LinearGradient style={[styles.evCard, { marginLeft: 20, borderRadius: 20, }]} colors={['rgba(0,0,0,.2)', 'rgba(0,0,0,.2)', 'rgba(0,0,0,.2)']}>
                                    <Image style={styles.evCard} source={appInfo && appInfo.applicationIcon ? { uri: appInfo.applicationIcon } : Images.default}></Image>
                                </LinearGradient>
                                <View style={{ flex: 1, alignItems: 'flex-start' }}>
                                    <Text style={styles.nameText}>{appInfo && appInfo.name}</Text>
                                    <Text style={styles.tagLineText}>{appInfo && appInfo.tagline}</Text>
                                </View>
                                <TouchableOpacity onPress={() => this.share(true)} style={styles.shareContainer}>
                                    <AntDesign name='sharealt' size={26} color='#000' />
                                </TouchableOpacity>
                            </View>
                            {/* <Text style={styles.contactText}>About {appInfo && appInfo.name}</Text> */}
                            {appInfo && appInfo.description && appInfo.description.length != 0 &&
                            ...
                            <View style={styles.evContainer1}>
                                <LinearGradient style={[styles.evCard, { marginLeft: 20 }]} colors={['rgba(0,0,0,.2)', 'rgba(0,0,0,.2)', 'rgba(0,0,0,.2)']}>
                                    <Image style={styles.evCard} source={evAppInfo && evAppInfo.applicationIcon ? { uri: evAppInfo.applicationIcon } : Images.default}></Image>
                                </LinearGradient>
                                <View style={{ flex: 1, alignItems: 'flex-start' }}>
                                    <Text style={styles.nameText}>{evAppInfo && evAppInfo.name}</Text>
                                    <Text style={styles.tagLineText}>{evAppInfo && evAppInfo.tagline}</Text>
                                </View>
                            ...
                            <View style={styles.contactContainContainer}>
                                <TouchableOpacity onPress={() => this.handleEmail()} style={styles.contactContainer}>
                                    <MaterialCommunityIcons name='email' size={16} color='white' />
                                    <Text style={styles.contactText2}>Email</Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => Linking.openURL('https://' + evAppInfo.websitelink)} style={styles.contactContainer}>
                                    <Entypo name='globe' size={16} color='white' />
                                    <Text style={styles.contactText2}>Website</Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => Linking.openURL(Platform.OS == 'ios' ? evAppInfo.appStoreLink : evAppInfo.googleStoreLink)} style={styles.contactContainer}>
                                    <Text style={styles.contactText3}>Download</Text>
                                </TouchableOpacity>
                            </View>
                            {/* <Text style={styles.contactText}>About Us</Text> */}
                            {evAppInfo && evAppInfo.description && evAppInfo.description.length != 0 &&
                                <View style={styles.webViewContainer}>
                                    <AutoHeightWebView
                                        style={styles.autoWebView}
                                        javaScriptEnabled={true}
                        ...
                        <Text style={styles.contactText5}>His Excellency Mohammed Ahmed Khalifa Al Suwaidi</Text>
                    </ScrollView> :
