...
import { StyleSheet, View, TouchableOpacity, Dimensions, FlatList, Text, Image, ScrollView, SafeAreaView, TextInput,RefreshControl } from 'react-native'
...
import { PERIOD_LIST, AUTHORS_LIST, AUTHORS_SEARCH } from '../../../common/endpoints'
...
class App extends Component {
    ...
    fetchBook(reset) {
        this.setState({isLoading:true,searchText:null,author:reset? []:this.state.author})
        Api('get', AUTHORS_LIST, { page: reset ? 1 : this.state.page }).then(async (response) => {
            if (response) {
                this.setState({
    ...
    onSearchPage(reset) {
        this.setState({ searchVisible: false, author:reset? []:this.state.author, isLoading: true })
        var base64 = require('base-64');
        var utf8 = require('utf8');
        var text = this.state.searchText;
        var bytes = utf8.encode(text);
        var encoded = base64.encode(bytes);
        Api('get', AUTHORS_SEARCH, { searchtext: encoded, page:reset ? 1 : this.state.page }).then((response) => {
            if (response) {
                this.setState({
    ...
    renderSearchModal = () => (
        <View style={styles.content2}>
            ...
            <View style={styles.textInputContainer}>
                ...
                />
                <View>
                    <TouchableOpacity style={styles.buyContainer} onPress={() => this.onSearchPage(true)}>
    ...
    renderBookItem({ item, index }) {
        return (
            <TouchableOpacity style={styles.cardGrid} onPress={() => this.props.navigation.navigate('AuthorDetail', { authorId: item&&item.authorId })}>
                ...
                <Text style={[styles.nameText, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]} ellipsizeMode={"middle"} numberOfLines={1}>{item.authorName}</Text>
                {/* <View style={[styles.prizeContainer, { flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }]}> */}
                <View style={styles.dateContainer}>
                    <Text style={[styles.authorText, { textAlign: 'left' }]} numberOfLines={1}> {item.birthDate}</Text>
                    <Text style={[styles.authorText, { textAlign: 'right' }]} numberOfLines={1}>{item.deathDate}</Text>
                </View>
            </TouchableOpacity>
        )
    }
    ...
    render() {
        return (
            <SafeAreaView style={styles.container}>
                <Text style={[styles.title, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{I18n.t("Author")}</Text>
                ...
                <ScrollView showsVerticalScrollIndicator={false}
                    ...
                    />
                    {!this.state.isLastBookPage && this.state.author && this.state.author.length > 0 &&
                        <View style={styles.indicatorContainer}>
                            <MaterialIndicator color={PRIMARY_COLOR} size={20} />
                        </View>}
                    {this.state.isLoading &&this.state.author&&this.state.author.length==0&&
