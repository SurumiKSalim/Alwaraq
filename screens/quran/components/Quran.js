...
import { StyleSheet, View, TouchableOpacity, Dimensions, FlatList, Text, Image, TextInput, ScrollView, SafeAreaView } from 'react-native'
...
class App extends Component {
    ...
    loadQuran(sura) {
        ...
        Api('get', QURAN, { pageid: sura ? 0 : this.state.page, sura: sura ? sura : 1 }).then((response) => {
            if (response) {
                this.setState({ quran: response.quranpage[0], loading: false, sura: response.quranpage[0].suraid })
    ...
    pageRight() {
        ...
        if (this.props.navigation.getParam('fromSearch') && page > 0) {
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, page, 1))
        }
    }
    ...
    pageLeft() {
        ...
        if (this.props.navigation.getParam('fromSearch') && page <= this.state.totalpages) {
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, page, 1))
        }
    }
    ...
    leftMost() {
        if (!this.state.fromSearch) {
            ...
            this.setState({ page: this.state.totalpages, pageIndex: this.state.totalpages - 1 })
            if (this.props.navigation.getParam('fromSearch')) {
                this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, this.state.totalpages, 1))
            }
        }
    }
    ...
    rightMost() {
        if (!this.state.fromSearch) {
            ...
            if (this.props.navigation.getParam('fromSearch')) {
                this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, 1, 1))
            }
        }
    }
    ...
    goToPage() {
        if (this.state.Text > 0 && this.state.Text <= 360) {
            this.setState({ page: this.state.Text, visible: false, invaliedPage: false })
        }
        else
        ...
        return (
            <View>
                <View style={styles.renderItemContainer}>
                    ...
                    <TouchableOpacity style={styles.renderItemSubContainer} onPress={() => this.loadQuran(item.suwarid)}>
                        <Text style={styles.nameText}>{parseInt(item.suwarid)}:{item.name}</Text>
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevState.page != this.state.page) {
            if (!this.state.fromSearch)
                ...
                this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, this.state.page, 1))
        }
    }
    ...
    render() {
        ...
        console.log('html',this.state.fromSearch ? response && utf8.decode(base64.decode(response)) : this.state.quran && utf8.decode(base64.decode(this.state.quran.page)))
        return (
            <GestureRecognizer
                ...
                <SafeAreaView style={styles.container}>
                    <View style={styles.titleContainer}>
                        <Text style={[styles.title,{textAlign:this.props.locale=='ar'?'right':'left'}]}>{I18n.t("Quran")}</Text>
                    ...
                    {!this.state.loading ?
                        <WebView
                            ...
                            source={{ html: this.state.fromSearch ? response && utf8.decode(base64.decode(response)) : this.state.quran && utf8.decode(base64.decode(this.state.quran.page)) }} /> :
