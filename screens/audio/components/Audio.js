...
class App extends Component {
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            headerLeft: (params.moduleListShow && <HeaderBackButton tintColor={PRIMARY_COLOR} onPress={() => params.this.stepBack()} />),
            headerTitle: (
    ...
    audioBookFetch() {
        ...
        Api('get', DOCUMENT_INFOS, { isAudioAvailable: 1, page: this.state.page }).then((response) => {
            if (response) {
                console.log('audio booksasss', response)
                this.setState({ isAudioBookPage: true, isAudioLastPage: response.isLastPage, popularDocumentList: this.state.popularDocumentList.concat(response.books), isAudioLoading: false, page: parseInt(this.state.page) + 1 })
            }
        })
    }
    ...
    stepBack() {
        if (this.state.article)
            ...
            this.setState({ moduleListShow: false, isAudioBookPage: false, popularDocumentList: [], page: 1 })
            this.props.navigation.setParams({
                moduleListShow: false,
            })
        }
    ...
    selection(item, info, article) {
        ...
        this.setState({ moduleListShow: true, info: info, selectedItem: item, article: article, moduleId: item.moduleId })
        this.props.navigation.setParams({
            moduleListShow: true,
        })
        // if (info) {
        ...
        // }
        // if (!info) {
        //     this.setState({ moduleListShow: true, article: article })
        // }
    }
    ...
    renderChildItem({ item, index }) {
        return (
            <TouchableOpacity onPress={() => this.selection(item, false, true)} style={styles.containerContent}>
                <View style={styles.renderItemImg}>
                    <Image style={[styles.card1, { width: width / 2.3 }]} source={{ uri: `data:image/jpeg;base64,${item.coverImage}` }}></Image>
                </View>
                <View style={styles.renderItemText1}>
    ...
    goBack() {
        this.setState({ article: false, isAudioBookPage: false, popularDocumentList: [], page: 1 })
    }
        ...
        return (
            <TouchableOpacity style={styles.cardGrid} onPress={() => this.props.navigation.navigate('Detailbuy', { data: item, fromPopular: true })}>
                ...
                <View style={{ flexDirection: 'row' }}>
                    <Text style={styles.nameText} ellipsizeMode={"middle"} numberOfLines={1}>{item.memberid == 0 ? item.name : item.name.concat(memberid)}</Text>
                </View>
            ...
            <SafeAreaView style={styles.container}>
                {!this.state.moduleListShow &&
                    <View style={styles.titleContainer}>
                        <Text style={[styles.title, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{I18n.t("Audio_Gallery")}</Text>
                ...
                <View style={styles.actionContainer}>
                    {this.props.fromLibrary && this.state.moduleListShow &&
                        <HeaderBackButton onPress={this.state.article ? () => this.setState({ article: false }) : () => this.setState({ moduleListShow: false, isAudioBookPage: false, popularDocumentList: [], page: 1,info:false })} />}
                    {this.state.info&&
                    <TouchableOpacity onPress={() => this.selection(this.state.selectedItem, false, false)} style={[styles.infoContainer, { alignItems: 'flex-end' }]}>
                        <MaterialCommunityIcons name='microsoft-xbox-controller-menu' color={PRIMARY_COLOR} size={23} />
                    </TouchableOpacity>}
                </View>
                ...
                {this.state.moduleListShow &&
                    <View style={{ flex: 1 }}>
                                ...
                                onMomentumScrollEnd={({ nativeEvent }) => {
                                    if (isCloseToBottom(nativeEvent)) {
                                        if (!this.state.isAudioBookPage) {
                                            ...
                                            !this.state.isAudioLoading && !this.state.isAudioLastPage &&
                                                this.audioBookFetch()
                                        }
                                    }
                                }}>
                        ...
                        {((this.props.isModuleListLoading && !this.state.info && !this.state.onLoadLoader) || (this.state.isAudioLoading && this.state.popularDocumentList && this.state.popularDocumentList.length == 0)) &&
                        ...
                        {this.state.info && !this.state.article &&
                            <View showsVerticalScrollIndicator={false} style={styles.fullFlux}>
                                ...
                                <TouchableOpacity onPress={() => this.setState({info:false,moduleListShow:false})}>
...
const styles = StyleSheet.create({
    ...
    nameText: {
        fontSize: 17,
        fontFamily: FONT_SEMIBOLD,
        flexShrink: 1, flexWrap: 'wrap', alignItems: 'center'
    },
    prizeContainer: {
        flexDirection: 'row',
        justifyContent: 'space-between',
