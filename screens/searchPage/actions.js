...
export function fetchSearchResult(
  ...
  return function (dispatch) {
    ...
    let page =
      store.getState().searchpage.page != null
        ? store.getState().searchpage.page
        : 1;
    let params = {
      WordForm: wordFormRadio,
      option: option2,
      offset: 1,
      searchtext: searchText,
      searchcolumn: subRadioValue,
      page: page,
      period: period,
      sortOrder: sortOrder,
      language: locale,
...
export function fetchSearchBookPage(
  item,
  navigation,
  page,
  firstRadioValue,
      ...
      if (response) {
        ...
        if (firstRadioValue != 1) {
          ...
          navigation.navigate('BookPage', {
            ...
            page: response.exsearchpage[0].pageid,
            item: item,
            itemResponse: response.exsearchpage[0],
            radioValue: firstRadioValue,
          });
        } else {
          navigation.navigate('Quran', {
            response: response.quransearchpage[0].pagecontent,
            fromSearch: true,
            totalpages: item.totalpages,
            page: response.quransearchpage[0].offset,
            pageId:response.quransearchpage[0].pageid,
            item: item,
            itemResponse: response.quransearchpage[0],
            radioValue: firstRadioValue,
          });
        }
      } else {
