...
import { Text, View, RefreshControl, StyleSheet, Dimensions, TextInput, FlatList, TouchableOpacity, Image, SafeAreaView, Platform } from 'react-native';
...
export class App extends React.Component {
        ...
        return {
            ...
            headerRight: (
                <View style={styles.headerRightContainer}>
                    <Text style={styles.headerText}>{params.this && params.this.state.demo[params.bookLanguage] && params.this.state.demo[params.bookLanguage].slice(0, 2)}</Text>
                    ...
                    <ModalDropdown
                        ...
                        onSelect={(idx, value) => params.this._dropdownOnSelect(idx, value)}
                    >
                    ...
                    {/* {params.this && params.this.props.navigation.getParam('fromLibrary') && */}
                    <TouchableOpacity style={{ marginLeft: 8 }} onPress={() => params.this.setState({ modalVisible: true })}>
    ...
    constructor(props) {
        ...
        this.state = {
            subjuctIndex: null,
            authorId: this.props.navigation.getParam('authorId')?this.props.navigation.getParam('authorId'):this.props.authorId,
            modalVisible: false,
            searchText: '',
            isSearchPage: false,
            bookLanguage:this.props.navigation.getParam('isJapanese')?5: undefined,
            demo: LanguageList,
            onLoadLoader: false,
            ...
            subjectName: this.props.navigation.getParam('authorName') ?
                this.props.navigation.getParam('authorName') : (data && data.subjectName) ?
                    data.subjectName : this.props.navigation.getParam('listType') == 'free' ?
                        I18n.t("FREE_BOOKS") : this.props.navigation.getParam('isJapanese') ?
                        I18n.t("Book") : I18n.t("New_Set_Books")
        }
    ...
    fetchData(bookLanguage) {
        ...
        this.props.dispatch(fetchBookList(this.state.subjectId, this.state.kind, this.props.locale == 'ar' ? 1 : 2, bookLanguage ? bookLanguage : this.state.bookLanguage, this.state.listType, this.state.authorId))
    }
    ...
    onSearch() {
        if (this.state.searchText !== '') {
            ...
            this.props.dispatch(resetBookList())
            this.props.dispatch(fetchSearch(encoded, this.state.subjectId,language,this.state.listType,this.state.authorId,this.state.bookLanguage ))
        }
    }
    ...
    onLoad() {
        ...
        if (!this.props.isLastPage) {
            if (!this.state.isSearchPage) {
                this.props.dispatch(fetchBookList(this.state.subjectId, this.state.kind, this.props.locale == 'ar' ? 1 : 2,this.state.bookLanguage, this.state.listType, this.state.authorId))
            }
            else {
                this.props.dispatch(fetchSearch(encoded, this.state.subjectId,language,this.state.listType,this.state.authorId,this.state.bookLanguage))
            }
    ...
    renderItem({ item }) {
        return (
            <TouchableOpacity style={styles.cardGrid} onPress={() => this.props.navigation.push('Detailbuy', { data: item, fromPopular: true })}>
                ...
                <Text style={[styles.nameText, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]} ellipsizeMode={"middle"} numberOfLines={1}>{item.memberid == 0 ? item.name : item.name.concat(item.memberid)}</Text>
                <View style={[styles.prizeContainer, { flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }]}>
                    ...
                    <TouchableOpacity onPress={() => item.inapp_free == 0 ? this.props.navigation.push('Detailbuy', { data: item, fromPopular: true }) : this.props.navigation.navigate('Subscribe')} style={[styles.prizeTextContainer, { width: I18n.locale == 'ar' ? 80 : 70, backgroundColor: Platform.OS == 'ios' ? '#F4E7E7' : '#fff' }]} >
        ...
        return (
            <SafeAreaView style={styles.container}>
                <View style={styles.titleContainer}>
                    <Text style={[styles.title, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{this.props.authorId?I18n.t("Book"):this.state.subjectName}<Text style={styles.subTitle}> ({this.props.total ? this.props.total : 0})</Text></Text>
                    <View style={[styles.emptyContainer, { alignSelf: this.props.locale == 'ar' ? 'flex-end' : 'flex-start',width:this.state.subjectName?this.state.subjectName.length*10:82 }]} />
                </View>
                <ScrollView showsVerticalScrollIndicator={false}
                    ...
                    <View style={styles.FlatListContainer2}>
                        {this.props.bookList && (!this.props.isLoading || this.state.onLoadLoader) &&
                            <FlatList
                        ...
                        {!this.props.isLoading && this.props.bookList && this.props.bookList.length === 0 &&
                ...
                >
                    <View style={[styles.modal, { height: Platform.OS == 'ios' ? 180 : 150 }]}>
                        {this.renderModal()}
                    </View>
                </Modal>
            </SafeAreaView>
