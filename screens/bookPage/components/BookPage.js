...
import { View, Text, Platform, StyleSheet, FlatList, TextInput, TouchableOpacity, Image, SafeAreaView, ScrollView, Dimensions } from 'react-native'
...
import { BOOK_PAGE, SEARCH_BOOK_PAGE, DOCUMENT_INFOS, IS_BOOK_BOUGHT, BOOK_READ, BOOKMARK, NEW_BOOK_PAGE } from '../../../common/endpoints'
...
import { UIActivityIndicator, DotIndicator, MaterialIndicator, BallIndicator, BarIndicator } from 'react-native-indicators';
...
class App extends Component {
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            ...
            headerLeft: (<HeaderBackButton tintColor={PRIMARY_COLOR} onPress={() => params.this.goBack()} />),
            // headerTitle: (
            ...
            headerRight: (
                <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity
                        ...
                        <MaterialCommunityIcons name='google-translate' color={PRIMARY_COLOR} size={26} />
                    </TouchableOpacity>
                    <TouchableOpacity
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            bookId: data ? data.bookid : item ? item.bookid : offlineData && offlineData.bookid,
            translatedText: null,
        ...
        console.log('iiii', this.props.navigation.getParam('item', null))
    }
        ...
        if (this.props.bookMark && this.props.bookMark.length > 0) {
            let obj = temp_arr.find((o, i) => {
                if (o.userId == userId) {
                    ...
                    let result = temp_arr && temp_arr[i] && temp_arr[i].bookMark && temp_arr[i].bookMark.find((o, i) => {
                    ...
                    if (!result) {
                        ...
                        temp_arr && temp_arr[i] && temp_arr[i].bookMark && temp_arr[i].bookMark.push({ bookId: parseInt(this.state.bookId), page: this.state.page })
                    }
                    ...
                    temp_arr = [{ userId: userId, bookMark: [{ bookId: parseInt(this.state.bookId), page: this.state.page }] }]
                    this.props.dispatch(fetchBookMark(temp_arr))
                }
            });
            ...
            let temp = [{ userId: userId, bookMark: [{ bookId: this.state.bookId, page: this.state.page }] }]
    ...
    audioPlay() {
        this.setState({ audioModal: this.state.audioLink ? false : true, isPlayAll: false, audioPlayer: false, audioLink: false, sound: false })
    ...
    goHome() {
        Api('get', DOCUMENT_INFOS, { bookId: this.state.bookId }).then((response) => {
            if (response) {
                this.setState({ totalpages: response.books[0].totalpages, fromSearch: false })
                this.props.navigation.navigate('Detailbuy', { data: response.books[0], fromSearch: true })
    ...
    pageTranlate(toLanguage) {
        ...
        // }
        // else {
        this.setState({ toLanguage: languageFormat[toLanguage], languageModal: false })
    ...
    readBook() {
        console.log('read')
        this.stopRead();
        if (!this.state.sound && this.state.pageStriped && this.state.pageStriped.length > 0) {
            // Api('get', BOOK_READ, { bookId: this.state.bookId, page: this.state.fromSearch ? this.state.fromSearchPageId : this.state.page }).then((response) => {
            //     if (response) {
    ...
    getBookPage() {
        ...
        if (this.props.bookMark && this.props.bookMark.length > 0) {
            let obj = this.props.bookMark.find((o, i) => {
                if (o.userId == userId) {
                    this.props.bookMark[i] && this.props.bookMark[i].bookMark && this.props.bookMark[i].bookMark.find((o, i) => {
    ...
    bookPageFetch(page) {
        ...
        Api('get', NEW_BOOK_PAGE, { bookId: this.state.bookId, page: page }).then((response) => {
            if (response) {
                console.log('hhhh222', response, page)
                this.setState({ htmlPage: response.page, pageAudioUrl: response.bookAudio, pageStriped: response.pageStriped })
            }
            this.setState({ webLoad: false })
        })
    }
    ...
    componentDidMount() {
        ...
        if (this.state.fromSearch) {
            this.setState({ bookId: this.props.navigation.getParam('item') && this.props.navigation.getParam('item').bookid })
        }
    ...
    pageRight() {
        ...
        if (this.state.page > 1) {
            console.log('1qqq')
            this.setState({ page: this.state.page - 1, pageIndex: parseInt(this.state.pageIndex) - 1 })
            this.bookPageFetch(page)
        }
        if (this.state.fromSearch && page > 0 && this.props.navigation.getParam('item')) {
            console.log('qqq')
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, page, this.props.navigation.getParam('radioValue')))
        }
    ...
    pageLeft() {
        ...
        if (this.state.page < parseInt(this.state.totalpages)) {
            console.log('1111')
            this.setState({ page: parseInt(this.state.page) + 1, pageIndex: parseInt(this.state.pageIndex) + 1 })
            this.bookPageFetch(page)
        }
        if (this.state.fromSearch && page <= this.state.totalpages && this.props.navigation.getParam('item')) {
            console.log('222')
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, page, this.props.navigation.getParam('radioValue')))
        }
        else if (this.state.fromSearch && this.state.page < parseInt(this.state.totalpages)) {
            console.log('3333')
            this.onSearchPage(this.state.page + 1)
        }
    }
    ...
    leftMost() {
        this.stopRead();
        this.setState({ page: this.state.totalpages, pageIndex: this.state.totalpages - 1, translatedText: null })
        this.bookPageFetch(this.state.totalpages)
        if (this.state.fromSearch && (this.state.page < parseInt(this.state.totalpages)) && this.props.navigation.getParam('item')) {
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, this.state.totalpages, this.props.navigation.getParam('radioValue')))
        }
        if (this.state.fromSearch && this.state.page < parseInt(this.state.totalpages)) {
    ...
    rightMost() {
        ...
        if (this.state.fromSearch && this.state.page > 1 && this.props.navigation.getParam('item')) {
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, 1, this.props.navigation.getParam('radioValue')))
        }
    ...
    pageSelect(pageid) {
        this.stopRead();
        this.setState({ page: pageid, visibleModal: false, pageIndex: pageid - 1, translatedText: null })
    ...
    goToPage() {
        ...
        if (txt > 0 && txt <= this.state.totalpages && !this.state.fromSearch) {
            ...
            this.setState({ page: txt, visible: false, invaliedPage: false, pageIndex: txt - 1 })
            this.bookPageFetch(txt)
        }
        if (this.state.fromSearch && txt <= parseInt(this.state.totalpages) && this.props.navigation.getParam('item')) {
            this.setState({ page: txt, visible: false, invaliedPage: false })
            this.props.dispatch(fetchSearchBookPage(this.props.navigation.getParam('item'), this.props.navigation, txt, this.props.navigation.getParam('radioValue')))
        }
        ...
        Api('get', DOCUMENT_INFOS, { bookId: this.state.bookId }).then((response) => {
            ...
            if (response) {
                Api('post', IS_BOOK_BOUGHT, formdata).then((res) => {
                    ...
                    if (res) {
                        if (response.books[0].inapp_free == 0 || (response.books[0].isBoughtIndividually == 0 && this.props.isPremium) || res.isBookBought) {
                            this.setState({ fromSearch: false, totalpages: pageInfo.totalpages, page: pageInfo.pageid, })
            ...
            <View>
                {item.child == 0 ?
                    <View style={styles.renderItemContainer}>
                        <TouchableOpacity style={styles.renderItemSubContainer} onPress={() => this.pageSelect(item.pageid)}>
                            {!this.props.isBookTreeLoading ?
                                <Text style={styles.nameText}>{item.name}</Text> :
                                <View style={styles.dotIndicatorContainer}>
                                    <DotIndicator color={PRIMARY_COLOR} size={10} style={styles.indicator} />
                                </View>}
                            <View style={styles.iconContainer}>
                                {!this.props.isBookTreeLoading ? <FontAwesome5 name='readme' color={PRIMARY_COLOR} size={20} /> :
                                    <UIActivityIndicator color={PRIMARY_COLOR} size={18} style={styles.indicator} />}
                            </View>
                        </TouchableOpacity>
                    </View> :
                    <View style={styles.renderItemContainer}>
                        <TouchableOpacity style={styles.renderItemSubContainer} onPress={() => this.chapterSelect(item.child)}>
                            {!this.props.isBookTreeLoading ?
                                <Text style={styles.nameText}>{item.name}</Text> :
                                <View style={styles.dotIndicatorContainer}>
                                    <DotIndicator color={PRIMARY_COLOR} size={10} style={styles.indicator} />
                            ...
                            <View style={styles.iconContainer}>
                                {!this.props.isBookTreeLoading ? <FontAwesome5 name='book-reader' color={PRIMARY_COLOR} size={20} /> :
                                    <UIActivityIndicator color={PRIMARY_COLOR} size={18} style={styles.indicator} />}
                            </View>
                        </TouchableOpacity>
                    </View>
   ...
   async playAudio(item, isPlayAll) {
      await this.setState({Text:item?.pageId})
        this.goToPage()
        this.setState({ audioModal: false, audioLink: item?.audioLink?item.audioLink:item, audioPlayer: true, sound: true, translatedText: null, isPlayAll: isPlayAll })
        this.props.navigation.setParams({
            sound: true,
        })
    }
    ...
    renderLanguageList(item, index) {
        return (
            <TouchableOpacity onPress={() => this.pageTranlate(index)} style={styles.audioContainer}>
                {/* <AntDesign name='sound' size={20} color={PRIMARY_COLOR} style={styles.audioContent} /> */}
                <Text numberOfLines={1} style={[styles.audioLink, { textAlign: 'center' }]}>{item}</Text>
            </TouchableOpacity>
        )
    }
    ...
    renderAudioItem(item, index) {
        return (
            <TouchableOpacity onPress={() => this.playAudio(item)} style={styles.audioContainer}>
                <AntDesign name='sound' size={20} color={PRIMARY_COLOR} style={styles.audioContent} />
                <Text numberOfLines={1} style={styles.audioLink}>{item?.audioTitle?item.audioTitle:'audio part '+parseInt(index+1)}</Text>
            </TouchableOpacity>
        )
    }
    ...
    renderBookMarkItem(item, index) {
        return (
            <TouchableOpacity onPress={() => this.setState({ page: item.pageId, bookMarkModal: false, bookRenderId: 'initial' })} style={styles.audioContainer}>
                <Text style={{ justifyContent: 'flex-start', height: '100%' }}>{index + 1}  :</Text>
                <View style={{ flex: 1 }}>
                    <Text numberOfLines={1} style={styles.audioLink}> {I18n.t("Page_No")}:{item.pageId} </Text>
                    {item.note != "" && <Text numberOfLines={1} style={[styles.audioLink, { color: PRIMARY_COLOR }]}> {I18n.t("Note")}:{item.note}</Text>}
                </View>
                <AntDesign onPress={() => this.bookMark('delete', item.bookmarkId)} name='delete' size={20} color={PRIMARY_COLOR} style={styles.audioContent} />
            </TouchableOpacity>
        )
    }
        ...
        <View style={styles.content}>
            <View style={[styles.modalClose, { paddingHorizontal: 15 }]}>
                {this.state.chapter && !this.props.isBookTreeLoading &&
                    <TouchableOpacity style={styles.backIcon} onPress={() => this.chapterBack()}>
                        <MaterialCommunityIcons name='arrow-left-bold-circle' color={PRIMARY_COLOR} size={30} />
                    </TouchableOpacity>}
                <TouchableOpacity style={styles.closeIcon} onPress={() => this.onClose()}>
    ...
    bookMark(action, bookmarkId, bookMarkPage) {
        this.setState({ bookMarkLoading: true, bookRenderId: action, translatedText: null })
        let formdata = new FormData()
        ...
        Api('post', BOOKMARK, formdata).then((response) => {
            ...
            if (response) {
                this.setState({ bookMarkList: bookMarkPage ? this.state.bookMarkList.concat(response.bookmarks) : response.bookmarks, isbookMarkLastPage: response.isLastPage, bookMarkPage: bookMarkPage ? bookMarkPage : 1 })
                if (action == 'add') {
    ...
    renderBookModal = () => (
        this.state.bookRenderId == 'initial' ?
            <View style={styles.bookContent}>
                <TouchableOpacity onPress={() => this.setState({ bookMarkModal: false, bookRenderId: 'initial', bookMarkList: [], bookMarkPage: 1 })} style={{ alignSelf: 'flex-end', height: 20 }}>
                    <AntDesign name='closecircle' size={20} color={PRIMARY_COLOR} />
                </TouchableOpacity>
                <Text style={styles.bookMarkText} onPress={() => this.setState({ bookRenderId: 'add' })}>{I18n.t("Add_Bookmark")}</Text>
                <Text style={styles.bookMarkText} onPress={() => this.bookMark()}>{I18n.t("View_Bookmarks")}</Text>
            </View> :
            this.state.bookRenderId == 'add' ?
                <View style={[styles.bookContent, { height: 180 }]}>
                    ...
                    <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]}>{I18n.t("Add_Bookmark")}</Text>
                    <TextInput
                    ...
                    />
                    <View style={[styles.submitContainer, { flexDirection: 'row', justifyContent: 'space-around' }]}>
                        ...
                        <TouchableOpacity onPress={() => this.bookMark('add')} style={styles.submitSubContainer1}>
                            <Text style={styles.submitText}>{I18n.t("Add")}</Text>
                        </TouchableOpacity>
                    </View>
                </View> : this.state.bookRenderId == 'success' ?
                    <View style={styles.bookContent}>
                        <TouchableOpacity onPress={() => this.setState({ bookMarkModal: false, bookRenderId: 'initial', bookMarkList: [], bookMarkPage: 1 })} style={{ alignSelf: 'flex-end', height: 20 }}>
                            <AntDesign name='closecircle' size={20} color={PRIMARY_COLOR} />
                        </TouchableOpacity>
                        <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]}>Success</Text>
                        <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]}>BookMark Added</Text>
                    </View> : this.state.bookRenderId == 'failed' ?
                        <View style={styles.bookContent}>
                            <TouchableOpacity onPress={() => this.setState({ bookMarkModal: false, bookRenderId: 'initial', bookMarkList: [], bookMarkPage: 1 })} style={{ alignSelf: 'flex-end', height: 20 }}>
                                <AntDesign name='closecircle' size={20} color={PRIMARY_COLOR} />
                            </TouchableOpacity>
                            <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]}>Failed</Text>
                            <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]}>Something Went Wrong</Text>
                        </View> :
                        <View style={[styles.bookContent, { height: height * .6 }]}>
                            <TouchableOpacity onPress={() => this.setState({ bookMarkModal: false, bookRenderId: 'initial', bookMarkList: [], bookMarkPage: 1 })} style={{ alignSelf: 'flex-end', height: 20 }}>
                                <AntDesign name='closecircle' size={20} color={PRIMARY_COLOR} />
                            </TouchableOpacity>
                            <Text style={[styles.bookMarkText, { color: '#9c9c9c' }]} onPress={() => this.bookMark('add')}>BookMarks</Text>
                                ...
                                <ScrollView showsVerticalScrollIndicator={false}
                                    ...
                                    onMomentumScrollEnd={({ nativeEvent }) => {
                                        if (isCloseToBottom(nativeEvent)) {
                                            if (!this.state.bookMarkLoading && !this.state.isbookMarkLastPage) {
                                                this.bookMark(null, null, parseInt(this.state.bookMarkPage) + 1)
                                            }
                                        }
                                    }}>
                                    <FlatList
                                        ...
                                        renderItem={({ item, index }) => this.renderBookMarkItem(item, index)}
                                        keyExtractor={(item, index) => index.toString()}
                                        showsVerticalScrollIndicator={false}
                                    /></ScrollView> :
                                <View style={{ flex: 1 }} />}
                            {this.state.bookMarkLoading && <MaterialIndicator color={PRIMARY_COLOR} size={30} />}
                        </View>
    );

    fetchResume() {
        this.setState({ page: this.state.temp_page, pageIndex: this.state.temp_page - 1, pageModal: false })
    ...
    renderTempPageModal = () => (
        <View style={styles.pageContent}>
            <View style={styles.modalClose}>
                <TouchableOpacity onPress={() => this.setState({ pageModal: false, invaliedPage: false })}>
                    <Icon name='ios-close-circle-outline' size={30} color='black' />
                </TouchableOpacity>
            </View>
            <Text style={styles.pageResume}>{I18n.t("Do_you_wish_resume")}</Text>
            <View style={[styles.submitContainer, { flexDirection: 'row', justifyContent: 'space-around' }]}>
                <TouchableOpacity onPress={() => this.setState({ pageModal: false })} style={styles.submitSubContainer1}>
                    <Text style={styles.submitText}>{I18n.t("Start_Over")}</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={() => this.fetchResume()} style={styles.submitSubContainer1}>
    ...
    renderPageModal = () => (
        <View style={styles.pageContent}>
            ...
            {this.state.invaliedPage &&
                <Text style={styles.invaliedPage}>{I18n.t("Enter_valid_numbers_in_English")}</Text>}
    ...
    onSearchPage(page) {
        ...
        console.log('2222', encoded, SEARCH_BOOK_PAGE, this.state.bookId, this.state.wordFormRadio, parseInt(this.state.secondRadioValue), page ? page : 1, encoded, this.state.totalpages)
        ...
        Api('get', SEARCH_BOOK_PAGE, { book: this.state.bookId, WordForm: this.state.wordFormRadio, option: parseInt(this.state.secondRadioValue), offset: page ? page : 1, searchtext: encoded, totalpages: this.state.totalpages }).then((response) => {
            if (response) {
                this.setState({
                    webLoad: false,
        ...
        <View style={styles.content2}>
            ...
            <View style={styles.radioContainerContent}>
                ...
                <View style={styles.flexContainer}>
                    <TouchableOpacity onPress={() => { this.setState({ secondRadioValue: 1 }) }} style={styles.radioContainer}>
                    ...
                    <TouchableOpacity onPress={() => { this.setState({ secondRadioValue: 2 }) }} style={styles.radioContainer}>
            ...
            <View style={styles.radioContainerContent}>
                ...
                <View style={styles.flexContainer}>
                    <TouchableOpacity onPress={() => { this.setState({ wordFormRadio: 1 }) }} style={styles.radioContainer}>
                    ...
                    <TouchableOpacity onPress={() => { this.setState({ wordFormRadio: 2 }) }} style={styles.radioContainer}>
                        <MaterialCommunityIcons name={this.state.wordFormRadio == 1 ? 'circle-outline' : 'circle-slice-8'} color={PRIMARY_COLOR} size={24} />
                        <Text style={styles.searchOptionText}>{I18n.t("With_Suffix_and_prefix")}</Text>
                    </TouchableOpacity>
                </View>
            </View>
            <View style={styles.textInputContainer}>
                <TextInput style={styles.searchText1}
                ...
                />
                <View>
                    <TouchableOpacity style={styles.buyContainer} onPress={() => this.onSearchPage()}>
    ...
    //         <TouchableOpacity style={styles.buttonCancel} onPress={() => this.setState({ languageModal: false })}>
    ...
    render() {
        let audioUrl = this.state.audioModal == 'pageAudio' ?
            this.state.pageAudioUrl : this.state.bookAudios
        let pageAudioLoading = this.state.pageAudioLoading
        let zoom = ["<div style='zoom:1.0;'>", "<div style='zoom:1.5;'>", "<div style='zoom:2;'>", "<div style='zoom:2.5;'>"]
        let extraheight = Platform.OS != 'ios' ? .06 : 0
        let bgColor = ["<div style='background-color:#fff;'>", "<div style='background-color:#fed7b2;'>", "<div style='background-color:#e1e1e1;'>"]
        let bgColors = ['fff', 'fed7b2', 'e1e1e1']
        let translaterTextStyle = this.state.toLanguage == 'ar' ?
        ...
        var kind = this.props.navigation.getParam('data') ? this.props.navigation.getParam('data').kind : 0
        const config = {
            velocityThreshold: 0.3,
            directionalOffsetThreshold: 80
        };
        let preStyle = zoom[this.state.zoomIndex] + bgColor[this.state.bgColorIndex]
        let bookPageUrl = BOOK_PAGE + '?book=' + this.state.bookId + '&page=' + this.state.page + '&kind=' + kind + '&size=' + (this.state.zoomIndex + 1) + '&bgColor=' + bgColors[this.state.bgColorIndex]
        return (
            <GestureRecognizer
                ...
                config={config}
                style={[styles.gestureContainer, { backgroundColor: '#' + bgColors[this.state.bgColorIndex] }]}
            >
                <SafeAreaView style={[styles.container, { backgroundColor: '#' + bgColors[this.state.bgColorIndex] }]}>
                    <DateCalc dateCalc={this.dateCalc} dateCalculator={this.state.dateCalculator} />
                    <View style={styles.subContainer}>
                        {this.state.webLoad &&
                            <View style={[styles.activityIndicatorContainer, { backgroundColor: '#' + bgColors[this.state.bgColorIndex] }]}>
                                <UIActivityIndicator color={PRIMARY_COLOR} size={30} style={styles.activityIndicator} />
                            </View>}
                        {this.state.webError &&
                        ...
                        {this.state.translatedText ?
                            <View style={{ flex: 1, marginBottom: this.state.audioLink ? (height * .1) + 130 : height * .1 }}>
                                <WebView
                                    showsVerticalScrollIndicator={false}
                                    ...
                                    source={{ html: translaterTextStyle + this.state.translatedText + '</div' }} />
                            </View> :
                            !this.props.navigation.getParam('offline') ?
                                <View style={{ height: this.state.webLoad ? '50%' : '100%', marginBottom: this.state.audioLink ? (height * .1) + 130 : height * .1 }}>
                                    {this.state.fromSearch !== 'failed' ?
                                        <WebView
                                            ...
                                            domStorageEnabled={true}
                                            onError={this.onError}
                                            style={{ marginBottom: this.state.audioLink ? (height * .1) + 130 : height * .1 }}
                                            onLoadStart={this.onLoadStart}
                                            onLoadEnd={this.onLoadEnd}
                                            mixedContentMode="compatibility"
                                            showsHorizontalScrollIndicator={false}
                                            source={this.state.fromSearch ? {
                                                html: this.state.exsearchpage ? preStyle + utf8.decode(base64.decode(this.state.exsearchpage)) :
                                ...
                                <View style={{ flex: 1, marginBottom: this.state.audioLink ? (height * .1) + 130 : height * .1 }}>
                                    <WebView
                                        ...
                                        mixedContentMode="compatibility"
                                        showsHorizontalScrollIndicator={false}
                                        source={{ html: preStyle + utf8.decode(base64.decode(this.state.offlineData.page[this.state.pageIndex].page)) }} />
                                </View>}
                        <View style={{ height: this.state.audioLink ? (height * .1) + 130 : height * .1, position: 'absolute', bottom: 0, right: 0, left: 0 }}>
                            {this.state.audioPlayer &&
                                <View style={styles.videoContainer}>
                                    ...
                                    <AntDesign
                                        ...
                                        name='closecircle'
                                        onPress={() => this.setState({ audioPlayer: false, audioLink: '', sound: false })}
                                        size={24}
                                        color={PRIMARY_COLOR} />
                                </View>}
                            {/* <View style={styles.videoContainer}>
                            ...
                            {this.props.navigation.getParam('radioValue') != 2 &&
                                <View style={[styles.bottomContainer, { height: (height * .1), backgroundColor: '#' + bgColors[this.state.bgColorIndex] }]}>
                                    ...
                                    <View style={styles.bottomSubContainer}>
                                        ...
                                        {!this.state.isAudioAvailable && this.state.languageId == 2 &&
                                            <TouchableOpacity onPress={() => this.readBook()} >
                                                {this.state.sound ? <MaterialCommunityIcons name='account-tie-voice-off' size={28} color={PRIMARY_COLOR} /> :
                                                    <MaterialCommunityIcons name='account-tie-voice' size={28} color={PRIMARY_COLOR} />}
                                            </TouchableOpacity>}
                                        <TouchableOpacity onPress={this.leftMost}>
                                            <MaterialCommunityIcons name='skip-previous-circle' color={PRIMARY_COLOR} size={36} />
                                        </TouchableOpacity>
                                        <TouchableOpacity onPress={this.pageLeft}>
                                            <MaterialCommunityIcons name='arrow-left-bold-circle' color={PRIMARY_COLOR} size={36} />
                                        </TouchableOpacity>
                                        {this.state.fromSearch !== 'failed' &&
                                            <TouchableOpacity onPress={() => this.setState({ visible: true, invaliedPage: false })} style={styles.pageContainer}>
                                                <Text>{this.state.page}/{this.state.totalpages}</Text>
                                            </TouchableOpacity>}
                                        <TouchableOpacity onPress={this.pageRight}>
                                            <MaterialCommunityIcons name='arrow-right-bold-circle' color={PRIMARY_COLOR} size={36} />
                                        </TouchableOpacity>
                                        <TouchableOpacity onPress={this.rightMost}>
                                            <MaterialCommunityIcons name='skip-next-circle' color={PRIMARY_COLOR} size={36} />
                                        </TouchableOpacity>
                        ...
                        <Modal
                            ...
                            backdropColor='black'
                            backdropOpacity={.5}
                            onBackButtonPress={() => this.setState({ audioModal: false, pageAudioLoading: null })}
                            onBackdropPress={() => this.setState({ audioModal: false, pageAudioLoading: null })}
                            style={styles.modal}>
                            <View style={styles.modalContainer1}>
                                        ...
                                        <View>
                                            {audioUrl && audioUrl.length > 1 && this.state.audioModal &&
                                                <TouchableOpacity
                                                    ...
                                                    <Text
                                                        numberOfLines={1}
                                                        style={[styles.audioLink, { color: PRIMARY_COLOR, textAlign: 'center' }]}>
                                                        PLAY ALL
                                                    </Text>
                                                </TouchableOpacity>}
                                            <FlatList
                                ...
                                <View style={styles.modalFooter}>
                                    <TouchableOpacity style={styles.buttonCancel} onPress={() => this.setState({ audioModal: false, pageAudioLoading: null })}>
                                        <Text style={styles.cancel}>CLOSE</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        ...
                        <Modal
                            ...
                            <View style={styles.modalContainer1}>
                                <View style={styles.modalHeaderLinks}>
                                    <View style={styles.audioContainer}>
                                        <Text numberOfLines={1} style={styles.translateInfo}>Translate to</Text>
                                    </View>
                                    <TouchableOpacity onPress={() => this.setState({ translatedText: null, languageModal: false })} style={styles.audioContainer}>
                                        <Text numberOfLines={1} style={[styles.audioLink, { textAlign: 'center' }]}>Default</Text>
                                    </TouchableOpacity>
                                    <FlatList
                                        data={LanguageList}
                                        ItemSeparatorComponent={this.renderHeader}
                                        renderItem={({ item, index }) => this.renderLanguageList(item, index)}
                                        keyExtractor={(item, index) => index.toString()}
                                        showsVerticalScrollIndicator={false}
                                    />
                                    <View style={styles.modalFooter}>
                        ...
                        <Modal
                            ...
                            <View style={styles.loginModalContainer}>
                                ...
                                <View style={styles.loginModalFooter}>
                                    <TouchableOpacity style={[styles.loginButtonCancel, { borderRightWidth: 1 }]} onPress={() => this.setState({ isModalVisible: false })}>
                                        <Text style={styles.cancel}>{I18n.t("CANCEL")}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => this.props.navigation.navigate('Profile') && this.setState({ isModalVisible: false })} style={styles.loginButtonCancel}>
                    ...
                    {this.state.pageAudioUrl && this.state.pageAudioUrl.length > 0 ?
                        <Icon
                            name={this.state.audioPlayer ?
                                'md-volume-mute-outline' :
                                'md-volume-high-outline'}
                            onPress={() => this.setState({ audioModal: this.state.audioPlayer ? false : 'pageAudio', isPlayAll: false, audioPlayer: false, audioLink: '' })}
