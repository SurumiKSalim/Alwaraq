...
import { Text, View, StyleSheet, RefreshControl, Dimensions, TextInput, FlatList, TouchableOpacity, Image, SafeAreaView, Platform } from 'react-native';
...
import { resetDashboardCategory, fetchDashboardCategory, fetchDocumentInfos, resetDocumentInfos, resetSearchModal, fetchBookAds, toogleLanguageModal } from '../actions'
...
const circleSvgPath = ({ position, canvasSize }) =>
    `M0,0H${canvasSize.x}V${canvasSize.y}H0V0ZM${-25 + position.x._value},${y_value + position.y._value
...
export class PopularScreen extends React.Component {
    ...
    subjectSelect(item, index) {
        if (this.props.fromLatest == 1) {
            ...
            if (this.state.subjuctIndex === index) {
                ...
                this.setState({ subjuctIndex: index, contemporarySubjectId: item.subjectId })
    ...
    onLoad() {
        ...
        if (this.props.fromLatest == 1) {
            if (!this.props.isLatestLastPage)
                this.props.dispatch(fetchDocumentInfos('latest', this.state.latestSubjectId, true))
        }
        else if (this.props.fromLatest == 0) {
            if (!this.props.isPopularLastPage)
                this.props.dispatch(fetchDocumentInfos('popular', this.state.popularSubjectId, true))
        }
        else {
            if (!this.props.isContemporaryLastPage)
                this.props.dispatch(fetchDocumentInfos('contemporary', this.state.contemporarySubjectId, true))
        }
    }
    ...
    fetchDashboard(booksads) {
        {
            booksads &&
                this.props.dispatch(fetchBookAds(booksads))
        }
        if (this.props.fromLatest == 1) {
            this.props.dispatch(resetDocumentInfos('latest'))
            this.props.dispatch(fetchDocumentInfos('latest', this.state.latestSubjectId))
        }
        else if (this.props.fromLatest == 0) {
            this.props.dispatch(resetDocumentInfos('popular'))
            this.props.dispatch(fetchDocumentInfos('popular', this.state.popularSubjectId))
        }
        else {
            this.props.dispatch(resetDocumentInfos('contemporary'))
            this.props.dispatch(fetchDocumentInfos('contemporary', this.state.contemporarySubjectId))
        }
    }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.locale != prevProps.locale) {
            I18n.locale = (this.props.user && this.props.user.language) ? this.props.user.language : this.props.locale
            this.fetchDashboard()
        }
    }
        ...
        return (
            <View>
                <Placeholder
                    ...
                    Right={props => (<PlaceholderMedia style={styles.categoryLoaderLeft} />)}>
    ...
    rendercatogory({ item, index }) {
        return (
            <TouchableOpacity style={[styles.rendercatogoryContainer, { backgroundColor: (this.state.subjuctIndex === index) ? PRIMARY_COLOR : '#FFF' }]} onPress={() => this.subjectSelect(item, index)}>
                <Text style={[styles.itemText, { color: (this.state.subjuctIndex === index) ? '#fff' : 'rgba(180, 0, 0, 0.78)' }]}>{item.subjectName}</Text>
            </TouchableOpacity>
        )
    }
    ...
    renderItem({ item }) {
        ...
        return (
            <TouchableOpacity style={styles.cardGrid} onPress={() => this.props.navigation.navigate('Detailbuy', { data: item, fromPopular: true })}>
                ...
                <Text style={[styles.nameText, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]} ellipsizeMode={"middle"} numberOfLines={1}>{item.memberid == 0 ? item.name : item.name.concat(memberid)}</Text>
                <View style={[styles.prizeContainer, { flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }]}>
                    ...
                    <TouchableOpacity style={[styles.prizeTextContainer, { width: I18n.locale == 'ar' ? 80 : 70 }]} onPress={() => item.inapp_free == 0 ? this.props.navigation.navigate('Detailbuy', { data: item, fromPopular: true }) : this.props.navigation.navigate('Subscribe')}>
            ...
            <SafeAreaView style={styles.container}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}
                    ...
                    onMomentumScrollEnd={({ nativeEvent }) => {
                        ...
                        <RefreshControl
                            refreshing={this.props.fromLatest ? this.props.isLatestLoading : this.props.isPopularLoading}
                            onRefresh={() => this.fetchDashboard(true)}
                            colors={["#F47424"]}
                        />
                    }>
                    <View style={styles.FlatListContainer2}>
                        {(this.props.fromLatest == 0 && this.props.popularDocumentList && (!this.props.isPopularLoading || this.state.onLoadLoader)) ||
                            (this.props.fromLatest == 1 && this.props.latestDocumentList && (!this.props.isLatestLoading || this.state.onLoadLoader)) ||
                            (this.props.fromLatest == 2 && this.props.ContemporaryDocumentList && (!this.props.isContemporaryLoading || this.state.onLoadLoader)) ?
                            <FlatList
                                style={{ marginBottom: 40 }}
                                showsVerticalScrollIndicator={false}
                                data={this.props.fromLatest == 1 ? this.props.latestDocumentList : this.props.fromLatest == 0 ? this.props.popularDocumentList : this.props.ContemporaryDocumentList}
                        ...
                        {((this.props.fromLatest == 0 && !this.props.isPopularLastPage) || (this.props.fromLatest == 1 && !this.props.isLatestLastPage)) &&
                            <MaterialIndicator color={PRIMARY_COLOR} size={20} />}
                        {((this.props.fromLatest == 0 && !this.props.isPopularLoading && this.props.popularDocumentList && this.props.popularDocumentList.length === 0) ||
                            (this.props.fromLatest == 1 && !this.props.isLatestLoading && this.props.latestDocumentList && this.props.latestDocumentList.length === 0) ||
                            (this.props.fromLatest == 2 && !this.props.isContemporaryLoading && this.props.ContemporaryDocumentList && this.props.ContemporaryDocumentList.length === 0)) &&
...
export class TabViewExample extends React.Component {
    ...
    fetchCategory() {
        ...
        this.props.dispatch(fetchDashboardCategory(this.props.locale == 'ar' ? 1 : 2))
    }
    ...
    carouselLoader(item) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate('Detailbuy', { data: item, fromPopular: true })} style={[{ backgroundColor: '#fff' }, this.state.size]} >
                <ImageBackground blurRadius={10} resizeMode='stretch' style={styles.adsImg} source={item.coverImage ? { uri: item.coverImage } : Images.default}>
                    <LinearGradient style={[styles.adsGradient, { flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }]} colors={['rgba(0,0,0,.2)', 'rgba(0,0,0,.2)', 'rgba(0,0,0,.2)']}>
                        <View style={[styles.sliderImg, { marginLeft: this.props.locale == 'ar' ? 0 : 20, marginRight: this.props.locale == 'ar' ? 20 : 0 }]}>
                        <Image
                        ...
                        <View style={styles.adsTextContainer}>
                            <Text numberOfLines={2} style={[styles.textName, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{item.name}</Text>
                            <Text numberOfLines={1} style={[styles.textAuthor, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{item.author}</Text>
                        </View>
                    </LinearGradient>
                </ImageBackground>
            </TouchableOpacity>
    ...
    render() {
        if (this.props.bookAds && this.props.latestAds && this.props.latestAds.length > 0) {
        ...
        var lastTabWidth = this.props.locale == 'ar' ? (width * 2 / 4) : (width * 2 / 4)
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', marginLeft: 10, marginRight: 15 }}>
                    <TouchableOpacity onPress={() => this.props.navigation.toggleDrawer()} style={{ zIndex: 10 }}>
                        <CopilotStep text={I18n.t("Click_here_to_view_Alwaraq_App_Options")} order={1} name={I18n.t("Menu")}>
                            <CopilotText>
                                <Entypo onPress={() => this.props.navigation.toggleDrawer()} name='menu' color={TITLE_COLOR} size={36} />
                            </CopilotText>
                        </CopilotStep>
                    ...
                    <View style={styles.headerRightContainer}>
                        <TouchableOpacity style={styles.menu} onPress={() => this.changeLanguage()}>
                            <CopilotStep text={I18n.t("Change_Language")} order={2} name={I18n.t("Language")}>
                                <CopilotText>
                                    <FontAwesome5 name='globe-asia' color={TITLE_COLOR} size={26} /></CopilotText>
                            </CopilotStep>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.onSearch()}>
                            <CopilotStep text={I18n.t("Search_for_books")} order={3} name={I18n.t("Search")}>
                                <CopilotText><FontAwesome name='search' color={TITLE_COLOR} size={30} /></CopilotText>
                            </CopilotStep>
                        </TouchableOpacity>
                    </View>
                </View>
                {this.props.bookAds && this.props.latestAds && this.props.latestAds.length > 0 &&
                    <View style={{ height: height * .25 }} onLayout={this._onLayoutDidChange}>
                        <Carousel
                            delay={2000}
                        ...
                        >
                            {arr && arr.map(item => {
                                return (
                                    this.carouselLoader(item)
                                );
                            })}
                            {this.carouselLoader(this.props.latestAds && this.props.latestAds[count] ? this.props.latestAds[count] : this.props.latestAds[0])}
                        </Carousel>
                        <AntDesign onPress={() => this.props.dispatch(fetchBookAds(false))} style={styles.close} name='closecircleo' size={20} color='#fff' />
                    </View>}
                <TabView
                    ...
                    renderScene={(route) => {
                        switch (route.route.key) {
                            case 'first':
                                return <PopularScreen navigation={this.props.navigation}
                                    value={this.state.value}
                                    categorySubjucts={this.props.categorySubjucts && this.props.categorySubjucts}
                                    popularDocumentList={this.props.popularDocumentList && this.props.popularDocumentList}
                            ...
                            case 'second':
                                return <PopularScreen navigation={this.props.navigation}
                                    categorySubjucts={this.props.categorySubjucts && this.props.categorySubjucts}
                                    latestDocumentList={this.props.latestDocumentList && this.props.latestDocumentList}
                            ...
                            case 'third':
                                return <PopularScreen navigation={this.props.navigation}
                                    categorySubjucts={this.props.categorySubjucts && this.props.categorySubjucts}
                                    ContemporaryDocumentList={this.props.ContemporaryDocumentList && this.props.ContemporaryDocumentList}
                        ...
                        return (
                            <TabBar
                                ...
                                renderLabel={({ route, focused, color }) => (
                                    <View style={{ width: route.key == "third" ? lastTabWidth : tabWidth, alignItems: this.props.locale == 'ar' ? 'center' : 'flex-end', marginLeft: this.props.locale == 'ar' ? 0 : route.key == "second" ? 30 : 0 }}>
                                        {route.key == "first" ?
                                            <CopilotStep text={I18n.t("Switch_between_tabs")} order={4} name={I18n.t("Popular_and_Latest")}>
                                                <CopilotText>
                                                    <Text style={[styles.titleText, { color: focused ? PRIMARY_COLOR : color }]}>
                                                        {route.title}
                                                    </Text>
                                                </CopilotText>
                                            </CopilotStep>
                                            : route.key == "second" ?
                                                <CopilotStep text={I18n.t("Switch_between_tabs1")} order={5} name={I18n.t("Popular_and_Latest1")}>
                                                    <CopilotText>
                                                        <Text style={[styles.titleText, { color: focused ? PRIMARY_COLOR : color }]}>
                                                            {route.title}
                                                        </Text>
                                                ...
                                                <CopilotStep text={I18n.t("Switch_between_tabs2")} order={6} name={I18n.t("Popular_and_Latest2")}>
                                                    <CopilotText>
                                                        <Text style={[styles.titleText, { color: focused ? PRIMARY_COLOR : color }]}>
                                                            {route.title}
                                                        </Text>
