...
import { StyleSheet, FlatList, View, TouchableOpacity, Image, Linking, Text, StatusBar, ScrollView, Platform, Dimensions, Alert } from 'react-native'
...
import RNIap, { purchaseUpdatedListener, purchaseErrorListener, acknowledgePurchaseAndroid} from 'react-native-iap'
...
class App extends Component {
        ...
        return {
            ...
            headerTitle: (
                <View style={styles.headerTitleContainer}>
                    <Image source={Images.logo} resizeMode='contain' style={styles.logoContainer} />
                </View>
            ),
            headerRight: (
    ...
    dataFetch() {
        ...
        Api('post', PREMIUM_SUBSCRIPTION, formdata).then((response) => {
            ...
            if (response && response.statusCode === 200) {
                ...
                SubscribeItems = Platform.OS == 'ios' ? response.info.map(({ iosProductId }) => iosProductId) : response.info.map(({ androidProductId }) => androidProductId);
                console.log('result', SubscribeItems)
            ...
            if (receipt) {
                ...
                try {
                    if (Platform.OS === 'android') {
                        SubscribeItems = await acknowledgePurchaseAndroid(purchase.purchaseToken);
                    }
                } catch (ackErr) {
    ...
    addPromo(purchase) {
        console.log('purchaseaa', purchase)
        let formdata = new FormData()
    ...
    getSubscriptionsAmount(index) {
        let amount = this.state.products && this.state.products.length > 0 && this.state.products[index].localizedPrice
        return amount
    }
    ...
    defaultPayment(index) {
        this.setState({ subscritionType: index + 1, isVisible: false }, () => this.buySubscribeItem(SubscribeItems[index]))
    }

    thirdPartyPayment(index, mode) {
        let paymentMethodId=this.state.subscriptionForms&&this.state.subscriptionForms[1]&&this.state.subscriptionForms[1].paymentMethodId
        let name=this.state.subscriptionForms&&this.state.subscriptionForms[1]&&this.state.subscriptionForms[1].name
        console.log('index:', index,mode)
        this.setState({ isVisible: false })
        if (this.props.user) {
            if (mode === 'etisalat') {
                console.log('mode:', mode)
                this.props.navigation.navigate('EtisalatPayment',{products:this.state.products,index:index,SubscribeItems:this.state.SubscribeItems,paymentMethodId:paymentMethodId,title:name})
            }
            else {
                setTimeout(() => {
                    Alert.alert(
    ...
    renderModalPassword = () => (
        <View style={styles.contents}>
            <View>
                ...
                <TouchableOpacity onPress={() => this.defaultPayment(this.state.index)} style={styles.modalSelectionContent1}>
                    <Text style={[styles.modalText, { color: !this.props.isSocialLogin ? TITLE_COLOR : '#D7DBDD' }]}>Play Store</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => this.thirdPartyPayment(this.state.index, 'card')} style={styles.modalSelectionContent1}>
                    <Text style={styles.modalText}>{this.state.subscriptionForms?this.state.subscriptionForms&&this.state.subscriptionForms[0]&&this.state.subscriptionForms[0].name:'Alwaraq Payment Gateway'}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => this.thirdPartyPayment(this.state.index, 'etisalat')} style={styles.modalSelectionContent1}>
                    <Text style={styles.modalText}>{this.state.subscriptionForms?this.state.subscriptionForms&&this.state.subscriptionForms[1]&&this.state.subscriptionForms[1].name:'Etisalat Payment Gateway'}</Text>
                </TouchableOpacity>
            </View>
            <TouchableOpacity onPress={() => this.setState({ isVisible: false })} style={styles.modalSelectionContent1}>
        ...
        return (
            <View style={styles.month}>
                <View style={styles.contentContainer}>
                    <View>
                        <Text style={styles.textMonth}>{products && products[index] && products[index].title}</Text>
                    </View>
                    <View>
                        <Text style={styles.amount}>{this.getSubscriptionsAmount(index)}</Text>
                        {/* <Text style={styles.per}>{I18n.t("per_month")}</Text> */}
                    </View>
                    <View style={styles.validityInfo}>
                        <Text style={styles.access}>{products && products[index] && products[index].description}</Text>
                        {/* <Text style={styles.billed}>{this.getSubscriptionsAmount(index)} {I18n.t("billed_for_one_month")}</Text> */}
                    </View>
                    <TouchableOpacity style={styles.subscribButton1} onPress={() => this.paymentMethod(index)}>
                        <Text style={styles.premiumButton}>{this.state.isSubscribing ? 'Please Wait..' : I18n.t("SUBSCRIBE")}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        )
                ...
                {!this.state.isLoading ?
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <View style={styles.content}>
                            ...
                            <Text style={styles.subHeader}>{I18n.t("Downgrade_or_upgrade_at_any_time")}</Text>
                        ...
                        <View style={styles.restoreContainer}>
                            <Text style={styles.restoreInfoContainer}>if you are already subscribed</Text>
                            <TouchableOpacity style={styles.Restore} onPress={() => this.restorePurchase()}>
                    ...
                    <BarIndicator style={{backgroundColor:'transparent'}} color={PRIMARY_COLOR} size={34} />}
            </SafeAreaView >
        )
    }
