...
import { View, Linking, Text, StyleSheet, Alert, TouchableOpacity, Image, SafeAreaView, ScrollView, Dimensions, TextInput } from 'react-native'
import Demo from '../../../mockData/homeData'
import images from '../../../assets/images'
import { PRIMARY_COLOR } from '../../../assets/color'
import Ionicons from "react-native-vector-icons/Ionicons"
import { FONT_REGULAR, FONT_BOLD, FONT_SEMIBOLD, FONT_MEDIUM, FONT_LIGHT } from '../../../assets/fonts'
import Images from '../../../assets/images'
...
class App extends Component {
        ...
        return {
            ...
            headerRight: (
                <View style={styles.headerContainer}>
                    <TouchableOpacity style={styles.headerRightContainer} onPress={() => params.this.props.navigation.navigate('Cart')}>
                        <Ionicons name='cart-outline' style={styles.iconContainer} size={26} color={PRIMARY_COLOR} />
                    </TouchableOpacity>
                </View>
            ),
            headerTitleStyle: {
    ...
    purchase() {
        ...
        formData.append('amount', this.state.buyDetails && this.state.buyDetails.totalPrice);
    ...
    buyNow(action) {
        ...
        let shippingId = this.props.shipping && this.props.shipping[0] && this.props.shipping[0].shippingId
        if (this.props.shipping && this.props.shipping[this.state.index]) {
            shippingId = this.props.shipping && this.props.shipping[this.state.index] && this.props.shipping[this.state.index].shippingId
        }
        if (data) {
            let formData = new FormData()
                ...
                .then((response) => {
                    ...
                    if (response.statusCode === 200) {
                        if (action == 'orderNow') {
                            ...
                            this.props.navigation.navigate('PaymentWebView', { secureUrlPayment: this.state.secureUrlPayment, orderId: this.state.orderId, productType: this.state.buyOption, totalPrice: totalPrice, refCode: this.state.promoCode })
                        }
                        else {
                            setTimeout(() => {
                                this.setState({
                                    buyModel: true, buyDetails: response,
                                    paymentMethods: response && response.paymentMethods && response.paymentMethods[0] && response.paymentMethods[0].id,
                                    prePurchaseUrl: response && response.paymentMethods && response.paymentMethods[0] && response.paymentMethods[0].link
                                })
                            }, 500)
                        }
                    }
        ...
        return (
            <TouchableOpacity style={styles.cardGrid}>
                ...
                <View style={styles.actionContainer}>
                    <TouchableOpacity onPress={() => this.props.navigation.navigate('AddressManager', { fromPurchase: true })} style={styles.remove}>
    ...
    priceView() {
        ...
        let price = this.state.buyOption == 1 ? parseFloat(data.price) : parseFloat(data.hardcoverprice)
        return (
            <TouchableOpacity style={styles.cardGrid}>
                ...
                <View style={styles.priceContainer}>
                    <View style={styles.priceSubContainer}>
                        ...
                        <Text style={[styles.addText, { textAlign: 'right' }]}>${parseInt(this.state.quantity) * price} </Text>
                    </View>
                    <View style={styles.priceSubContainer}>
                        <Text style={styles.addText}>{I18n.t("Discount")}</Text>
                        <Text style={[styles.addText, { textAlign: 'right', color: 'green' }]}>-${buyDetails && buyDetails.discountPrice}</Text>
                    </View>
                    <View style={styles.priceSubContainer}>
                        <Text style={styles.addText}>{I18n.t("Delivery_Charge")}</Text>
                        <Text style={[styles.addText, { textAlign: 'right' }]}>+${buyDetails && buyDetails.shippingCost}</Text>
                    </View>
                    <View style={styles.priceSubContainer}>
                        <Text style={styles.addText}>{I18n.t("Tax")}</Text>
                        <Text style={[styles.addText, { textAlign: 'right' }]}>+${buyDetails && buyDetails.tax}</Text>
                    </View>
                    <View style={styles.priceSubContainer}>
                        <Text style={styles.addText}>{I18n.t("Amount_payable")}</Text>
                        <Text style={[styles.addText, { textAlign: 'right' }]}>${buyDetails && buyDetails.totalPrice}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        )
    ...
    isvaildData() {
        ...
        //     return false;
        // }
        if (this.state.buyOption != 1 && this.props.shipping && this.props.shipping.length == 0) {
    ...
    render() {
        ...
        return (
            <SafeAreaView style={styles.container}>
                <ScrollView style={{ zIndex: 20 }} scrollEventThrottle={1} showsVerticalScrollIndicator={false}>
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <View style={{marginLeft:8}}>
                            ...
                            <Text numberOfLines={1} style={styles.authorNameText}>{data && data.author}</Text>
                        </View>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', zIndex: 20 }}>
                        <View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: width - 190 }}>
                                <Text numberOfLines={1} style={styles.addText}>{I18n.t("Price")}</Text>
                                <Text numberOfLines={1} style={styles.addText}>: $ {price}</Text>
                            </View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: width - 190 }}>
                                <Text numberOfLines={1} style={styles.addText}>{I18n.t("weight")}</Text>
                                <Text numberOfLines={1} style={styles.addText}>: {data.weightkgs == 0 ? 'N/A' : data.weightkgs + ' Kg'}</Text>
                            </View>
                        </View>
                        <ModalDropdown
                            options={Qty}
                        ...
                        >
                            <View style={styles.category}>
                                <Text style={styles.input}>{I18n.t("Qty")} : {this.state.quantity}</Text>
                    ...
                    {data && categoryList.map(item => {
                        if (data && data[item.key] != 0) {
                            return (
                                <TouchableOpacity onPress={() => this.setState({ buyOption: item.value })} style={styles.typeContainer}>
                                    <Text style={styles.highlighted2}>{I18n.t(item.label)} {data && data[item.key]}</Text>
                                    <Fontisto name={this.state.buyOption == item.value ? 'radio-btn-active' : 'radio-btn-passive'} size={20} color={PRIMARY_COLOR} />
                                </TouchableOpacity>
                            );
                        }
                        else return null
                    })}
                    {this.state.errors.radioError && this.state.errors.radioError == true && <View style={styles.infoTextContainer}><Text style={styles.infoText}>Selection Required</Text></View>}
                    <View style={styles.textInputContainer}>
                        ...
                        />
                        <View>
                            <TouchableOpacity onPress={() => this.buyNow()} style={styles.buyContainer}>
                    ...
                    {this.state.buyOption != 1 && this.state.buyOption != 4 &&
                        <View>
                            {this.props.shipping && this.props.shipping.length != 0 ?
                                this.addressView(this.props.shipping[this.state.index] ? this.props.shipping[this.state.index] : this.props.shipping[0]) :
                                <TouchableOpacity onPress={() => this.props.navigation.navigate('AddressManager', { addressEditor: 0 })} style={styles.addContainer}>
                                    <Text style={styles.addContainerText}>{I18n.t("Add_a_Address")}</Text>
                                </TouchableOpacity>}
                        </View>}
                    {this.state.errors.addressError && this.state.errors.addressError == true && <View style={styles.infoTextContainer}><Text style={styles.infoText}>{I18n.t("Address_Required")}</Text></View>}
                    {this.priceView()}
                    {buyDetails && buyDetails.paymentMethods && buyDetails.paymentMethods.map(item => {
                        return (
                            <View>
                                <Text style={styles.highlighted3}>{item.description}</Text>
                                <TouchableOpacity onPress={() => this.setState({ paymentMethods: item.id, paymentLink: item.link, prePurchaseUrl: item.link })} style={[styles.typeContainer, { marginTop: 4 }]}>
                                    <Text style={styles.highlighted2}>{item.name}</Text>
                                    <Fontisto name={this.state.paymentMethods == item.id ? 'radio-btn-active' : 'radio-btn-passive'} size={20} color={PRIMARY_COLOR} />
                                </TouchableOpacity>
                            </View>
                        );
                    })}
                ...
                <View style={styles.bottomBar}>
                    <View>
                        <Text style={styles.price}>$ {this.state.buyDetails && this.state.buyDetails.totalPrice ? this.state.buyDetails.totalPrice : price}</Text>
                        <Text style={styles.aedPrice}>AED {this.state.buyDetails?.totalPriceAED}</Text>
                    </View>
                    {this.addCartView()}
                    <TouchableOpacity onPress={() => this.onSubmit()} style={styles.continue}>
