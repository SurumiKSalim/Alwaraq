...
import { View, Linking, Text, TextInput, Alert, StyleSheet, KeyboardAvoidingView, Image, Switch, SafeAreaView, TouchableOpacity, Dimensions, ScrollView } from 'react-native'
...
import { FONT_REGULAR, FONT_SEMIBOLD, FONT_MEDIUM, FONT_LIGHT } from '../../../assets/fonts'
import Images from '../../../assets/images'
import ActionSheet from 'react-native-actionsheet'
import { resetUser, postChangeProfileImage, postNotificationToggle, sendFCMToken, postChangeLanguage, fetchChangeName, promoSubscription } from '../../login/actions'
...
class App extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps != this.props && this.props.user != prevProps.user) {
            // this.configureFCM()
            this.setState({ updateUserName: this.props.user && this.props.user.fullname })
    ...
    onLanguageChange() {
        I18n.locale = this.state.currentLanguage ? this.state.currentLanguage : this.state.user && this.state.user.language
        this.setState({ visibleModal: false, selection: 0 })
        this.props.dispatch(postChangeLanguage({
    ...
    onSubmit() {
        if (this.state.currentpassword && this.state.newpassword && this.state.confirmpassword) {
            if (this.state.newpassword == this.state.confirmpassword) {
                if (this.state.currentpassword == this.state.confirmpassword) { this.setState({ validate: 'Old password and new password should be different' }) }
                else {
                    this.setState({ validate: null, visibleModal: null, selection: 0 }, () => {
                        setTimeout(() => {
                            this.props.dispatch(postChangePassword({
                                currentPassword: this.state.currentpassword,
                                newPassword: this.state.newpassword
                            }))
                        }, 1000)
                        this.setState({ currentPassword: null, newPassword: null, confirmpassword: null })
                    })
                }
            }
            else { this.setState({ validate: 'New password and confirm password should be same' }) }
        }
        else { this.setState({ validate: 'Fields cannot be empty' }) }
    }
    ...
    renderModalEdit = () => (
        ...
        <View style={styles.content}>
            ...
            <View style={styles.txtinput}>
                <TextInput
                    value={this.state.updateUserName != null ? this.state.updateUserName : this.state.usernameEditField}
            ...
            <TouchableOpacity onPress={this.state.updateUserName !== '' && this.changeFullname} style={styles.modalButton}>
                <View style={styles.TouchableContentView}>
                    <Text style={styles.modalButtonFont}>{this.props.isLoading ? I18n.t("changing") : 'Submit'}</Text>
                </View>
            </TouchableOpacity>
        </View>
    );
        ...
        <View style={styles.content}>
            <View style={styles.modalClose}>
                {this.state.selection != 0 &&
                    <TouchableOpacity style={styles.iconContainer} onPress={() => this.setState({ selection: 0 })}>
                        <Ionicons name='arrow-back' size={30} color='black' />
                    </TouchableOpacity>}
                <TouchableOpacity style={styles.iconContainer1} onPress={() => this.setState({ visibleModal: false, selection: 0 })}>
            ...
            {this.state.selection == 0 &&
                <View>
                    ...
                    <TouchableOpacity onPress={() => !this.props.isSocialLogin && this.setState({ selection: 1 })} style={styles.modalSelectionContent1}>
                        <Text style={[styles.modalText, { color: !this.props.isSocialLogin ? TITLE_COLOR : '#D7DBDD' }]}>{I18n.t("Change_Password")}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => this.setState({ selection: 2 })} style={styles.modalSelectionContent1}>
                        <Text style={styles.modalText}>{I18n.t("Change_Language")}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => this.setState({ selection: 3 })} style={styles.modalSelectionContent2}>
            ...
            {this.state.selection == 1 &&
                <View>
                    ...
                    <TouchableOpacity onPress={this.onSubmit} style={styles.modalButton}>
                        <View style={styles.TouchableContentView}>
                            <Text style={styles.modalButtonFont}>{this.props.passwordLoading ? I18n.t("changing") : I18n.t("Change_Password")}</Text>
                        </View>
                    </TouchableOpacity>
                </View>}
            {this.state.selection == 2 &&
                <View>
                    ...
                    <RadioButton options={options} lang={this.onLanguageChangeProps} locale={this.getUsedTranslation()} />
                    <TouchableOpacity onPress={this.onLanguageChange} style={styles.modalButton}>
                        <View style={styles.TouchableContentView}>
            ...
            {this.state.selection == 3 &&
                <View>
                    ...
                    <TouchableOpacity onPress={this.onPromoSubmit} style={styles.modalButton}>
                        <View style={styles.TouchableContentView}>
                            <Text style={styles.modalButtonFont}>{this.state.promoLoading ? 'please Wait . . .' : 'Apply'}</Text>
                        </View>
                    </TouchableOpacity>
                </View>}
            {this.state.selection == 4 &&
                <View>
                    ...
                    <View style={[styles.txtinput, { height: 100 }]}>
                        ...
                        <Text style={[styles.modalTxtInput1, { fontSize: 12 }]}>YOU CAN ENJOY PREMIUM CONTENTS NOW</Text>
                    </View>
                    <TouchableOpacity onPress={() => this.setState({ visibleModal: false, selection: 0 })} style={styles.modalButton}>
                        <View style={styles.TouchableContentView}>
            ...
            {this.state.selection == 5 &&
                <View>
                    ...
                    <View style={[styles.txtinput, { height: 100 }]}>
                        ...
                        <Text style={[styles.modalTxtInput1, { fontSize: 12 }]}>Something went wrong</Text>
                    </View>
                    <TouchableOpacity onPress={() => this.setState({ visibleModal: false, selection: 0 })} style={styles.modalButton}>
                        <View style={styles.TouchableContentView}>
                            <Text style={styles.modalButtonFont}>{I18n.t("Ok")}</Text>
        ...
        return (
            !this.props.user ? <Login navigation={this.props.navigation} /> :
                <SafeAreaView style={styles.container}>
                    <ScrollView showsVerticalScrollIndicator={false} style={styles.notificationIcon}>
                        <View style={styles.containerView}>
                            <View style={styles.profileContainer}>
                                <Image style={styles.card}
                                    source={user != null ? { uri: user.profile_pic } : Images.avatar} />
                                <View style={styles.editProfContainer}>
                                    <Touch style={styles.editProf} onPress={() => this.changePofilePic()}>
                                        <TouchableOpacity onPress={() => this.changePofilePic()}>
                                            <Text numberOfLines={1} style={styles.editProfText}>{I18n.t("Edit_Picture")}</Text>
                                        </TouchableOpacity>
                                    </Touch>
                                </View>
                            </View>
                            ...
                            <View style={styles.profInfoContainer}>
                                <View style={styles.profInfoContainerContent}>
                                    <Text style={styles.profName} numberOfLines={1}>{user ? user.fullname : '-'}</Text>
                                    <MaterialIcons name='edit' style={styles.userNameIcon} size={19} color='grey' onPress={() => this.setState({ isVisible: true, updateUserName: this.props.user.fullname })} />
                                </View>
                                {this.props.isPremium &&
                                    <View style={styles.profInfoContainerContent}>
                                        <Text style={styles.profPlace} numberOfLines={2}>Premium Member</Text>
                                        <AntDesign name='star' style={styles.userNameIcon} size={16} color={'#D4AF37'} />
                                    </View>}
                        ...
                        <View style={styles.subContainer}>
                            <View style={styles.subContainerContents}>
                                <FontAwesome name={'bell-o'} color={PRIMARY_COLOR} size={25} style={styles.notificationIcon} />
                            ...
                            <TouchableOpacity onPress={() => !this.props.passwordLoading && this.setState({ visibleModal: true })} style={styles.subContainerContents}>
                                <SimpleLineIcons name={'user'} color={PRIMARY_COLOR} size={25} style={styles.notificationIcon} />
                                <Text style={styles.optionText}>{this.props.passwordLoading ? I18n.t("changing") : I18n.t("My_Account")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('ShareEarn')} style={styles.subContainerContents}>
                                <Ionicons name={'wallet-outline'} color={PRIMARY_COLOR} size={28} style={styles.favIcon} />
                                <Text style={styles.optionText}>{I18n.t("My_Wallet")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookMarks')} style={styles.subContainerContents}>
                                <Ionicons name={'bookmark-outline'} color={PRIMARY_COLOR} size={28} style={styles.favIcon} />
                                <Text style={styles.optionText}>{I18n.t("My_Bookmarks")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Downloads')} style={styles.subContainerContents}>
                                <Ionicons name={'ios-download-outline'} color={PRIMARY_COLOR} size={28} style={styles.favIcon} />
                                <Text style={styles.optionText}>{I18n.t("Downloads")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Purchased')} style={styles.subContainerContents}>
                                <Feather name='unlock' style={styles.favIcon} color={PRIMARY_COLOR} size={26} />
                                <Text style={styles.optionText}>{I18n.t("Purchased")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('AddressManager')} style={styles.subContainerContents}>
                                <FontAwesome name={'address-card-o'} color={PRIMARY_COLOR} size={22} style={styles.notificationIcon} />
                                <Text style={styles.optionText}>{I18n.t("Shipping_Address")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Favourites')} style={styles.subContainerContents}>
                                <MaterialIcons name={'favorite-border'} color={PRIMARY_COLOR} size={28} style={styles.favIcon} />
                                <Text style={styles.optionText}>{I18n.t("Favourites")}</Text>
                                <View style={styles.arrowIcon}>
                                    <Ionicons name={'ios-arrow-forward'} color={PRIMARY_COLOR} size={30} />
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => Linking.openURL(HELP_AND_SUPPORT + `${this.state.languageId}`)} style={styles.subContainerContents}>
                                <SimpleLineIcons name={'info'} color={PRIMARY_COLOR} size={25} style={styles.notificationIcon} />
                        ...
                        <Modal
                            ...
                            onBackButtonPress={() => this.setState({ visibleModal: false, selection: 0 })}
                            onBackdropPress={() => this.setState({ visibleModal: false, selection: 0 })}
                            animationOutTiming={1000}
                            style={styles.bottomModal}
                        >
                            <KeyboardAvoidingView behavior='padding' enabled keyboardVerticalOffset={Platform.OS == 'ios' ? keybordHeight : -250} style={[styles.KeyboardViewContainer, { height: this.state.selection == 1 ? 420 : 300 }]}>
                        ...
                        <Modal
                            ...
                            style={styles.bottomModal}
                        >
                            <KeyboardAvoidingView style={styles.KeyboardViewContainer2} behavior='padding' enabled keyboardVerticalOffset={Platform.OS == 'ios' ? 0 : -300} >
...
const styles = StyleSheet.create({
    ...
    switch: {
        ...
        transform: [{ scaleX: Platform.OS === 'ios' ? .9 : 1 },
