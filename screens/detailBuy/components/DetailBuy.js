...
import { Clipboard, View, Linking, Platform, Text, StyleSheet, Image, SafeAreaView, TouchableOpacity, FlatList, ScrollView, Dimensions } from 'react-native'
...
import { OFFLINE_DOWNLOAD, CHECK_FAVOURITES, DOCUMENT_INFOS, BOOK_BOUGHT, IS_BOOK_BOUGHT, APP_INFO } from '../../../common/endpoints'
import * as Progress from 'react-native-progress';
import Api from '../../../common/api'
import _ from 'lodash';
import { fetchBookDownload, resetBookDownload, fetchCoverImage, fetchIsBookBought, fetchPdfDownload, fetchCoverImg, fetchAudioDownload, fetchAudioRemove } from '../actions'
...
class App extends Component {
            ...
            headerRight: (
                <View style={styles.headerContainer}>
                    {params.isVideoAvailable &&
                        <TouchableOpacity onPress={() => params.this.audioPlay(true)} style={styles.headerRightContainer}>
                            <Feather name='video' size={24} color={PRIMARY_COLOR} />
                        </TouchableOpacity>}
                    {params.isAudioAvailable &&
                        <TouchableOpacity onPress={() => params.this.audioPlay()} style={styles.headerRightContainer}>
                            {params.sound ? <FontAwesome5 name='volume-mute' size={24} color={PRIMARY_COLOR} /> :
                                <FontAwesome5 name='volume-up' size={24} color={PRIMARY_COLOR} />}
                        </TouchableOpacity>}
                    <TouchableOpacity style={styles.headerRightContainer} onPress={() => params.this.props.navigation.navigate('TimeLine', { data: params.this.state.data })}>
                        <MaterialCommunityIcons name={'timeline-clock-outline'} color={PRIMARY_COLOR} size={28} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.headerRightContainer} onPress={() => params.this.share()}>
                        <MaterialCommunityIcons name={'share-variant'} color={PRIMARY_COLOR} size={26} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.headerRightContainer} onPress={() => params.this.favourites()}>
                        <MaterialIcons name={params.isFavouriste ? 'favorite' : 'favorite-border'} color={PRIMARY_COLOR} size={28} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => params.this.setState({ moreInfo: true })}>
                        <MaterialIcons name={'info-outline'} color={PRIMARY_COLOR} size={28} />
                    </TouchableOpacity>
                </View>
            ),
            headerStyle: {
    ...
    constructor(props) {
        ...
        let dataKey = ['publisher', 'publishingdate', 'publisheraddr', 'researcher', 'translator', 'totalpages', 'subject']
        let dataValue = data ? [data.publisher, data.publishingdate, data.publisheraddr, data.researcher, data.translator, data.totalpages, data.subjectName] : ''
        var url = [{ latitude: parseFloat(data && data.latitude), longitude: parseFloat(data && data.longitude), query: data && data.locationName, zoom: 5 }]
        ...
        this.state = {
            ...
            popularDocumentList: '',
            data: data,
            bookAudios: data && data.audioArray ? data.audioArray : data && data.bookAudio,
    ...
    audioPlay(isVideo) {
        ...
        if (this.props.isPremium || data.inapp_free == 0) {
            if (data.isBoughtIndividually == 0 || (data.isBoughtIndividually == 1 && this.state.isAudioBought)) {
                if (isVideo) {
            ...
            if (item.videoLink) {
                ...
                if (match) {
                    ...
                    await fetch(`https://player.vimeo.com/video/${VIMEO_ID}/config`)
                        .then(res => res.json())
                        .then(res =>
                            videoPlayLink = res.request.files.hls.cdns[res.request.files.hls.default_cdn].url.trim(),
                        );
    ...
    playAudio(item, isPlayAll) {
        this.setState({ audioModal: false, audioLink: item, audioPlayer: true, isPlayAll: isPlayAll })
        this.props.navigation.setParams({
            sound: true,
        })
    }
    ...
    renderAudioItem(item, index) {
        return (
            <TouchableOpacity onPress={this.state.isVideo ? () => this.playVideo(item) : () => this.playAudio(item?.audioLink?item.audioLink:item)} style={styles.audioContainer}>
                {this.state.isVideo ? <Feather name='video' size={20} color={PRIMARY_COLOR} style={styles.audioContent} /> :
                    <AntDesign name='sound' size={20} color={PRIMARY_COLOR} style={styles.audioContent} />}
                    {this.state.isVideo ? 
                <Text numberOfLines={1} style={styles.audioLink}>{ 'Video part'} :{index + 1}</Text>:
    ...
    favourites() {
        if (this.props.user) {
            if (this.state.isFavouriste != null) {
                ...
                this.props.dispatch(fetchAddFavourites(actions, this.state.data && this.state.data.bookid))
                if (actions == 'add')
    ...
    checkFavourites() {
        if (this.props.user) {
            Api('get', CHECK_FAVOURITES, { bookId: this.state.data && this.state.data.bookid }).then((response) => {
                if (response) {
    ...
    fetchAppInfo() {
        Api('get', APP_INFO, { appId: 1, language: this.props.locale == 'ar' ? 1 : 2 }).then((response) => {
            if (response) {
    ...
    componentDidMount() {
        ...
        this.checkBookBought()
        Api('get', DOCUMENT_INFOS, { subjectId: this.state.data && this.state.data.subjectId, kind: this.state.data && this.state.data.kind }).then(async (response) => {
            if (response) {
                this.setState({ isLoading: false })
                this.setState({ popularDocumentList: _.without(response.books, this.state.data) })
            }
            this.fetchAppInfo()
        })
    ...
    bookDownload(data, reset, loop) {
        ...
        if (!this.state.downloadStop) {
            ...
            if (!(this.props.offlinebook.find(o => o.bookid == data.bookid)) || loop) {
                ...
                Api('get', OFFLINE_DOWNLOAD, { book: data.bookid, page: page }).then((response) => {
                    let percent = this.props.page / (totalpages / 10) < 1 ? this.props.page / (totalpages / 10) : 1
    ...
    readBook(data) {
        if ((data.isBoughtIndividually == 0 && this.props.isPremium) || data.inapp_free == 0 || this.state.isBookBought) {
            this.props.navigation.navigate('BookPage', { data: data, fromSearch: false })
        }
        else
        ...
        <ScrollView
            ...
            <View style={styles.coverImgContainer}>
                <LinearGradient
                    ...
                    <Image
                        style={styles.card1}
                        source={data && data.coverImage ? { uri: data.coverImage } : Images.default} />
                </LinearGradient>
            </View>
            <View style={styles.titleContainer}>
                ...
                <Text
                    onPress={() => this.gotoSubject('authorBooks', { authorId: data && data.authorid, authorName: data && data.author })}
                    numberOfLines={1}
            ...
            <View
                ...
                {this.state.dataKey.map((item, index) => {
                    return (
                        this.state.dataValue[index] != "NA" && this.state.dataValue[index] != "2000-01-01" &&
                        <View
                            style={[styles.ratingContainer, { flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }]}>
                            <Text
                                numberOfLines={1}
                                style={styles.authorNameText1}>
                                {I18n.t(item)} :
                                </Text>
                            <Text
                ...
                <View
                    style={styles.moreInfoTextContainer}>
                    <Text
                        style={styles.moreInfoText1}>
                        {data.addinfo}
                    </Text>
                </View>
            </View>
    ...
    renderVideoModal = () => (
        <View showsVerticalScrollIndicator={false} style={styles.content}>
            <View style={{ flex: 1, justifyContent: 'center', }}>
                {videoPlayLink != '' ?
                    <Videoplay navigation={this.props.navigation} videoLink={videoPlayLink} /> :
                    <Text style={{ fontFamily: FONT_SEMIBOLD, textAlign: 'center' }}>{I18n.t('Something_went_wrong_Try')}</Text>}
            </View>
            <View style={styles.modalFooter1}>
                <TouchableOpacity style={styles.buttonCancel1} onPress={() => this.closeVideoModal()}>
    ...
    renderModalContent = () => (
        <View style={styles.loaderContainer}>
            ...
            <View style={styles.modalFooter}>
                <TouchableOpacity style={styles.buttonCancel} onPress={() => this.setState({ maxLimit: false })}>
    ...
    renderItem({ item }) {
        ...
        return (
            <TouchableOpacity style={[styles.cardRelated, { marginHorizontal: 5 }]}
                onPress={() => this.props.navigation.replace('Detailbuy', { data: item })}>
                ...
                <Text style={[styles.nameText, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]} ellipsizeMode={"middle"} numberOfLines={2}>{item.memberid == 0 ? item.name : item && item.name.concat(memberid)}</Text>
    ...
    bookCheck(data, action, isDownloadCheck, dualoption) {
        if (data.isBoughtIndividually == 1) {
            if (this.state.isBookBought) {
                isDownloadCheck ? this.downloadInitiate(data, action) : this.bookAction(data, dualoption)
            }
            else
    ...
    downloadPdf(data, imgPath) {
        RNFetchBlob
            ...
            .fetch('GET', data.bookPDFs && data.bookPDFs[0] && data.bookPDFs[0].pdfFile, {
                //some headers ..
            })
        ...
        if (data && data.bookAudio && data.bookAudio.length > 0) {
            RNFetchBlob
                ...
                .progress({ count: 10 }, (received, total) => {
                    ...
                    this.setState({ percent: ((counter / data.bookAudio.length) + ((received / total) / data.bookAudio.length)) })
                })
                .then((res) => {
                    if ((counter) < data.bookAudio.length) {
                        ...
                        if (audio_Array.length == data.bookAudio.length) {
                            this.props.dispatch(fetchAudioDownload(data, audio_Array, imgPath))
                        }
                    }
                })
        }
                ...
                <SafeAreaView style={styles.container}>
                    <ScrollView showsVerticalScrollIndicator={false} style={styles.ScrollContainer} >
                        <View style={styles.subContainer}>
                            <View style={styles.coverImgContainer}>
                                <Image style={styles.card} resizeMode='contain' source={data.coverImage ? { uri: data.coverImage } : Images.default}></Image>
                            </View>
                            <View style={styles.titleContainer}>
                                <Text
                                    style={styles.nameTexts}>
                                ...
                                <Text
                                    onPress={() => this.props.navigation.push('AuthorDetail', { authorId: data && data.authorid })}
                                    numberOfLines={1}
                                    style={[styles.authorNameText, { textDecorationColor: TITLE_COLOR, color: TITLE_COLOR }]}>
                                    {data.author}
                                </Text>
                                <Text
                                    onPress={() => this.gotoSubject('subject', data)}
                                ...
                                {data.isBoughtIndividually == 1 &&
                                    <View>
                                        {this.state.isBookBought ?
                                            <Text style={styles.prizeText1}>You Have Already Purchased This Book</Text> :
                                            <Text style={styles.prizeText1}>{data.inapp_free == 0 ? I18n.t("Free_book") : I18n.t("Price") + ': $' + price}</Text>
                                        }
                                    </View>}
                                <View style={{ flexDirection: 'row', }}>
                                    {data.isBoughtIndividually == 1 && data.inapp_free != 0 && (this.state.isBookBought != null || !this.props.user) && !this.state.isBookBought &&
                                        <TouchableOpacity style={styles.subscribeContainer} onPress={this.props.user ? () => this.props.navigation.navigate('Purchase', { data }) : () => this.props.navigation.navigate('Login', { goBack: true })}>
                                            <Text style={styles.prizeText}>{I18n.t("Buy_this_Book")}</Text>
                                        </TouchableOpacity>}
                                    {data.isBoughtIndividually == 0 &&
                                        <TouchableOpacity style={styles.subscribeContainer} onPress={() => !this.props.isPremium && this.props.navigation.navigate('Subscribe')}>
                                            <Text style={styles.prizeText}>{!this.props.isPremium ? I18n.t("Subscribe") : I18n.t("Subscribed")}</Text>
                                        </TouchableOpacity>}
                                </View>
                            ...
                            <View style={styles.likeCommentContainer}>
                                <TouchableOpacity onPress={this.props.user ? () => this.addLike() : () => this.setState({ isModalVisible: true })} style={styles.likeContainer}>
                                    <Text style={[styles.likeCountText, { color: this.props.isLiked ? PRIMARY_COLOR : '#9c9c9c' }]}>{this.props.totalLikes ? this.props.totalLikes : 0}</Text>
                                    <AntDesign name={this.props.isLiked ? 'like1' : 'like2'} size={22} color={this.props.isLiked ? PRIMARY_COLOR : '#9c9c9c'} />
                                    <Text style={[styles.commentText, { color: this.props.isLiked ? PRIMARY_COLOR : '#9c9c9c' }]}>{I18n.t("like")}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => this.props.dispatch(fetchCommentModal(true))} style={styles.likeContainer}>
                                    <Text style={styles.likeCountText}>{this.props.totalComments ? this.props.totalComments : 0}</Text>
                        ...
                        <Text style={[styles.relatedBooks, { textAlign: this.props.locale == 'ar' ? 'right' : 'left' }]}>{I18n.t("Related_books")}</Text>
                        ...
                        {!this.state.isLoading && this.state.popularDocumentList != '' ?
                            <View>
                                <FlatList
                                    ...
                                    ListFooterComponent={
                                        <TouchableOpacity style={[styles.cardImage, { backgroundColor: '#ECECEC', marginLeft: 5 }]} onPress={() => this.onPressMethod()}>
                                            <Text style={styles.nameText} numberOfLines={2}>{I18n.t("see_more")}</Text>
                                        </TouchableOpacity>}
                                    keyExtractor={(item, index) => index.toString()}
                                />
                            </View> :
                            <View style={{ flexDirection: this.props.locale == 'ar' ? 'row-reverse' : 'row' }}>
                                {this.loader()}
                            </View>
                        }
                    ...
                    <View style={styles.footerContainer}>
                        <View style={styles.footer}>
                            <View style={styles.footerPrizeContainer}>
                                <TouchableOpacity onPress={this.props.user ? () => this.addLike() : () => this.setState({ isModalVisible: true })} style={styles.footerRatingContainer}>
                                    <Text style={[styles.likeCountText, { color: this.props.isLiked ? PRIMARY_COLOR : '#9c9c9c' }]}>{this.props.totalLikes ? this.props.totalLikes : 0}</Text>
                                    <AntDesign name={this.props.isLiked ? 'like1' : 'like2'} size={22} color={this.props.isLiked ? PRIMARY_COLOR : '#9c9c9c'} />
                                </TouchableOpacity>
                            </View>
                        ...
                        {(action == 'Read_now' || action == 'Read_Pdf') ?
                            <TouchableOpacity style={styles.buyContainer} onPress={() => this.goToDownload(data, action)}>
                                <AntDesign name={'download'} color={'#515456'} size={18} />
                                {(this.state.percent > 0 && this.state.percent < 1) ?
                                    <Text style={styles.percentContainer}>{Math.floor(this.state.percent * 100)}%</Text> :
                                    <Text style={styles.downloadText}>{!(this.props.offlinebook.find(o => o.bookid == data.bookid)) ? I18n.t("Download") : I18n.t("Downloads")}</Text>}
                            </TouchableOpacity> :
                            <View style={{ width: 65 }}></View>}
                        {dualoption &&
                            <TouchableOpacity style={styles.buyContainer2} onPress={() => this.bookCheck(data, null, null, true)}>
                                <AntDesign name={'pdffile1'} color={'#fff'} size={18} />
                            </TouchableOpacity>}
                        <TouchableOpacity style={[styles.buyContainer1, { width: dualoption ? '30%' : '40%' }]} onPress={() => this.bookCheck(data)}>
                            {action == 'Read_Pdf' &&
                                <AntDesign name={'pdffile1'} style={{ marginRight: 8 }} color={'#fff'} size={26} />}
                            <Text style={styles.buyText}>{action == 'Read_Pdf' ? I18n.t('Read_now') : I18n.t(action)}</Text>
                        </TouchableOpacity>
                    </View>
                    <Modal
                    ...
                    <Modal
                        ...
                        <View style={styles.modalContainer1}>
                            {!this.state.isVideo && this.state.bookAudios && this.state.bookAudios.length == 0 &&
                                <Text style={styles.audioInfoText}>No Audios Found</Text>}
                            <ScrollView style={styles.modalHeaderLinks}>
                                {!this.state.isVideo && this.state.bookAudios && this.state.bookAudios.length > 1 &&
                                    <TouchableOpacity
                                        onPress={() => this.playAudio(this.state.bookAudios[0]?.audioLink?this.state.bookAudios[0]?.audioLink:this.state.bookAudios[0], true)}
                                ...
                                <FlatList
                                    data={this.state.isVideo ? this.state.videoBook : this.state.bookAudios}
                            ...
                            <View style={styles.modalFooter1}>
                                <TouchableOpacity style={styles.buttonCancel1} onPress={() => this.setState({ audioModal: false })}>
                                    <Text style={styles.cancel}>{I18n.t("close")}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    ...
                    <Modal
                        ...
                        <View style={styles.modalContainer}>
                            ...
                            <View style={styles.modalFooter}>
                                <TouchableOpacity style={[styles.buttonCancel, { borderRightWidth: 1 }]} onPress={() => this.setState({ isModalVisible: false })}>
                                    <Text style={styles.cancel}>{I18n.t("CANCEL")}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => this.props.navigation.navigate('Profile') && this.setState({ isModalVisible: false })} style={styles.buttonCancel}>
                                    <Text style={styles.subscribe}>{I18n.t("Login")}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    </Modal>
                    <PdfList pdfListModal={this.state.pdfListModal}
                        data={data.bookPDFs}
