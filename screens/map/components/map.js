...
class App extends Component {
  ...
  componentDidMount() {
    let fromIbnAwards = this.props.navigation.getParam('fromIbnAwards');
    ...
    if (!this.props.navigation.getParam('data')) {
      ...
      this.setState({
        feed: this.props.navigation.getParam('data'),
        isLoading: false,
      });
    }
  }
  ...
  fitToMarkersToMap(location) {
    if (location && location.length > 0) {
      ...
      this.map.fitToSuppliedMarkers(
        location.map((trip, index) => index.toString()),
        true,
      );
    }
  }
  ...
  onLoad() {
    ...
    if (!this.state.isBookLoading && !this.state.isLastPage) {
      ...
      Api('get', DOCUMENT_INFOS, {
        ...
        ibAwards:fromIbnAwards?1:undefined 
      }).then(async response => {
        if (response) {
          this.setState({
            bookResponse: this.state.bookResponse
              ? this.state.bookResponse.concat(response.books)
              : response.books,
            isBookLoading: false,
    ...
    return (
      <TouchableOpacity
        ...
        <View style={styles.renderItemImg}>
          <Image
            style={[styles.card1, {width: width / 2.6}]}
            ...
            this.state.feed.map(
              (item, index) => (
                <Marker
                  ...
                  title={
                    item.totalBooks
                      ? 'Available Books :' + item.totalBooks
                      : item.query
                  }
